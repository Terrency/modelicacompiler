# Modelica 标准库 - 可执行模型清单

生成时间: 2026-03-13T01:59:06.335Z

## 统计概览

- **可执行模型总数**: 2273
- **定义**: 有 `equation` 段的 `model` 定义

## 可执行模型列表

### Blocks (276 个)

- Modelica.ActuatorWithNoise
- Modelica.Blocks
- Modelica.Blocks.Abs
- Modelica.Blocks.Acos
- Modelica.Blocks.Adaptors
- Modelica.Blocks.Add
- Modelica.Blocks.Add3
- Modelica.Blocks.And
- Modelica.Blocks.And
- Modelica.Blocks.Asin
- Modelica.Blocks.Atan
- Modelica.Blocks.Atan2
- Modelica.Blocks.BandLimitedWhiteNoise
- Modelica.Blocks.BooleanChange
- Modelica.Blocks.BooleanConstant
- Modelica.Blocks.BooleanPassThrough
- Modelica.Blocks.BooleanPulse
- Modelica.Blocks.BooleanReplicator
- Modelica.Blocks.BooleanStep
- Modelica.Blocks.BooleanTable
- Modelica.Blocks.BooleanToInteger
- Modelica.Blocks.BooleanToReal
- Modelica.Blocks.BooleanValue
- Modelica.Blocks.Change
- Modelica.Blocks.ChangingEdge
- Modelica.Blocks.CombiTable1Ds
- Modelica.Blocks.CombiTable1Dv
- Modelica.Blocks.CombiTable2DBase
- Modelica.Blocks.CombiTable2Ds
- Modelica.Blocks.CombiTable2Dv
- Modelica.Blocks.CombiTimeTable
- Modelica.Blocks.Constant
- Modelica.Blocks.Continuous
- Modelica.Blocks.ContinuousClock
- Modelica.Blocks.ContinuousMean
- Modelica.Blocks.Cos
- Modelica.Blocks.Cosh
- Modelica.Blocks.Cosine
- Modelica.Blocks.CosineVariableFrequencyAndAmplitude
- Modelica.Blocks.CriticalDamping
- Modelica.Blocks.DeadZone
- Modelica.Blocks.DeMultiplex
- Modelica.Blocks.DeMultiplex2
- Modelica.Blocks.DeMultiplex3
- Modelica.Blocks.DeMultiplex4
- Modelica.Blocks.DeMultiplex5
- Modelica.Blocks.DeMultiplex6
- Modelica.Blocks.Der
- Modelica.Blocks.Derivative
- Modelica.Blocks.Discrete
- Modelica.Blocks.DiscreteBlock
- Modelica.Blocks.Division
- Modelica.Blocks.Edge
- Modelica.Blocks.Edge
- Modelica.Blocks.ExampleCTable
- Modelica.Blocks.Exp
- Modelica.Blocks.Exponentials
- Modelica.Blocks.ExpSine
- Modelica.Blocks.Extractor
- Modelica.Blocks.ExtractSignal
- Modelica.Blocks.FallingEdge
- Modelica.Blocks.FallingEdge
- Modelica.Blocks.Feedback
- Modelica.Blocks.Filter
- Modelica.Blocks.Filter
- Modelica.Blocks.FirstOrder
- Modelica.Blocks.FirstOrderHold
- Modelica.Blocks.FixedDelay
- Modelica.Blocks.FlowToPotentialAdaptor
- Modelica.Blocks.From_bar
- Modelica.Blocks.From_day
- Modelica.Blocks.From_deg
- Modelica.Blocks.From_degC
- Modelica.Blocks.From_degF
- Modelica.Blocks.From_degRk
- Modelica.Blocks.From_gps
- Modelica.Blocks.From_hour
- Modelica.Blocks.From_kmh
- Modelica.Blocks.From_kWh
- Modelica.Blocks.From_litre
- Modelica.Blocks.From_minute
- Modelica.Blocks.From_rpm
- Modelica.Blocks.Gain
- Modelica.Blocks.GlobalSeed
- Modelica.Blocks.Greater
- Modelica.Blocks.GreaterEqual
- Modelica.Blocks.GreaterEqualThreshold
- Modelica.Blocks.GreaterThreshold
- Modelica.Blocks.Harmonic
- Modelica.Blocks.Hysteresis
- Modelica.Blocks.IntegerChange
- Modelica.Blocks.IntegerConstant
- Modelica.Blocks.IntegerPassThrough
- Modelica.Blocks.IntegerReplicator
- Modelica.Blocks.IntegerStep
- Modelica.Blocks.IntegerTable
- Modelica.Blocks.IntegerToBoolean
- Modelica.Blocks.IntegerToReal
- Modelica.Blocks.IntegerValue
- Modelica.Blocks.Integrator
- Modelica.Blocks.Interaction
- Modelica.Blocks.Interfaces
- Modelica.Blocks.Internal
- Modelica.Blocks.Internal
- Modelica.Blocks.InverseBlockConstraints
- Modelica.Blocks.KinematicPTP
- Modelica.Blocks.KinematicPTP2
- Modelica.Blocks.Less
- Modelica.Blocks.LessEqual
- Modelica.Blocks.LessEqualThreshold
- Modelica.Blocks.LessThreshold
- Modelica.Blocks.LimIntegrator
- Modelica.Blocks.Limiter
- Modelica.Blocks.LimPID
- Modelica.Blocks.LimPID
- Modelica.Blocks.LinearDependency
- Modelica.Blocks.Log
- Modelica.Blocks.Log10
- Modelica.Blocks.LogFrequencySweep
- Modelica.Blocks.Logical
- Modelica.Blocks.LogicalDelay
- Modelica.Blocks.LogicalSwitch
- Modelica.Blocks.LowpassButterworth
- Modelica.Blocks.Math
- Modelica.Blocks.MathBoolean
- Modelica.Blocks.MathInteger
- Modelica.Blocks.MatrixGain
- Modelica.Blocks.Max
- Modelica.Blocks.Mean
- Modelica.Blocks.Min
- Modelica.Blocks.MinMax
- Modelica.Blocks.Multiplex
- Modelica.Blocks.Multiplex2
- Modelica.Blocks.Multiplex3
- Modelica.Blocks.Multiplex4
- Modelica.Blocks.Multiplex5
- Modelica.Blocks.Multiplex6
- Modelica.Blocks.MultiProduct
- Modelica.Blocks.MultiSum
- Modelica.Blocks.MultiSwitch
- Modelica.Blocks.MultiSwitch
- Modelica.Blocks.MultiSwitch
- Modelica.Blocks.MVdiscrete
- Modelica.Blocks.Nand
- Modelica.Blocks.Nand
- Modelica.Blocks.Noise
- Modelica.Blocks.Nonlinear
- Modelica.Blocks.Nor
- Modelica.Blocks.Nor
- Modelica.Blocks.NormalNoise
- Modelica.Blocks.Not
- Modelica.Blocks.Not
- Modelica.Blocks.OnDelay
- Modelica.Blocks.OnOffController
- Modelica.Blocks.Or
- Modelica.Blocks.Or
- Modelica.Blocks.PadeDelay
- Modelica.Blocks.PartialNoise
- Modelica.Blocks.PI
- Modelica.Blocks.PID
- Modelica.Blocks.PolarToRectangular
- Modelica.Blocks.PotentialToFlowAdaptor
- Modelica.Blocks.Power
- Modelica.Blocks.Pre
- Modelica.Blocks.Product
- Modelica.Blocks.Product
- Modelica.Blocks.Pulse
- Modelica.Blocks.Pythagoras
- Modelica.Blocks.RadioButtonSource
- Modelica.Blocks.Ramp
- Modelica.Blocks.RealPassThrough
- Modelica.Blocks.RealToBoolean
- Modelica.Blocks.RealToInteger
- Modelica.Blocks.RealValue
- Modelica.Blocks.RectangularToPolar
- Modelica.Blocks.RectifiedMean
- Modelica.Blocks.Replicator
- Modelica.Blocks.RisingEdge
- Modelica.Blocks.RootMeanSquare
- Modelica.Blocks.Routing
- Modelica.Blocks.RSFlipFlop
- Modelica.Blocks.Sampler
- Modelica.Blocks.SampleTrigger
- Modelica.Blocks.SawTooth
- Modelica.Blocks.SecondOrder
- Modelica.Blocks.Show
- Modelica.Blocks.Sign
- Modelica.Blocks.Sin
- Modelica.Blocks.Sinc
- Modelica.Blocks.Sine
- Modelica.Blocks.SineVariableFrequencyAndAmplitude
- Modelica.Blocks.Sinh
- Modelica.Blocks.SlewRateLimiter
- Modelica.Blocks.Sources
- Modelica.Blocks.Sqrt
- Modelica.Blocks.StandardDeviation
- Modelica.Blocks.StateSpace
- Modelica.Blocks.StateSpace
- Modelica.Blocks.Step
- Modelica.Blocks.Sum
- Modelica.Blocks.Sum
- Modelica.Blocks.SVdiscrete
- Modelica.Blocks.Switch
- Modelica.Blocks.Tables
- Modelica.Blocks.Tan
- Modelica.Blocks.Tanh
- Modelica.Blocks.TerminateSimulation
- Modelica.Blocks.Timer
- Modelica.Blocks.TimeTable
- Modelica.Blocks.To_bar
- Modelica.Blocks.To_day
- Modelica.Blocks.To_deg
- Modelica.Blocks.To_degC
- Modelica.Blocks.To_degF
- Modelica.Blocks.To_degRk
- Modelica.Blocks.To_gps
- Modelica.Blocks.To_hour
- Modelica.Blocks.To_kmh
- Modelica.Blocks.To_kWh
- Modelica.Blocks.To_litre
- Modelica.Blocks.To_minute
- Modelica.Blocks.To_rpm
- Modelica.Blocks.TotalHarmonicDistortion
- Modelica.Blocks.TransferFunction
- Modelica.Blocks.TransferFunction
- Modelica.Blocks.Trapezoid
- Modelica.Blocks.TriggeredAdd
- Modelica.Blocks.TriggeredMax
- Modelica.Blocks.TriggeredSampler
- Modelica.Blocks.TriggeredTrapezoid
- Modelica.Blocks.TruncatedNormalNoise
- Modelica.Blocks.UniformNoise
- Modelica.Blocks.UnitConversions
- Modelica.Blocks.UnitDelay
- Modelica.Blocks.Utilities
- Modelica.Blocks.VariableDelay
- Modelica.Blocks.VariableLimiter
- Modelica.Blocks.Variance
- Modelica.Blocks.WrapAngle
- Modelica.Blocks.Xor
- Modelica.Blocks.Xor
- Modelica.Blocks.ZeroCrossing
- Modelica.Blocks.ZeroOrderHold
- Modelica.BooleanNetwork1
- Modelica.BusUsage
- Modelica.BusUsage_Utilities
- Modelica.Controller
- Modelica.Densities
- Modelica.DrydenContinuousTurbulence
- Modelica.Examples
- Modelica.Filter
- Modelica.FilterWithDifferentiation
- Modelica.FilterWithRiseTime
- Modelica.ImpureRandom
- Modelica.IntegerNetwork1
- Modelica.Interaction1
- Modelica.InverseModel
- Modelica.LogicalNetwork1
- Modelica.Modulation
- Modelica.MotorWithCurrentControl
- Modelica.Noise
- Modelica.NormalDensity
- Modelica.NormalNoiseProperties
- Modelica.Part
- Modelica.Parts
- Modelica.PID_Controller
- Modelica.RealNetwork1
- Modelica.Rectifier12pulseFFT
- Modelica.Rectifier6pulseFFT
- Modelica.SinCosEncoder
- Modelica.SlewRateLimiter
- Modelica.TotalHarmonicDistortion
- Modelica.UniformDensity
- Modelica.UniformNoiseProperties
- Modelica.Utilities
- Modelica.WeibullDensity

### Clocked.BooleanSignals.NonPeriodic (4 个)

- Modelica.Clocked.BooleanSignals.NonPeriodic.BooleanChange
- Modelica.Clocked.BooleanSignals.NonPeriodic.ClockToBoolean
- Modelica.Clocked.BooleanSignals.NonPeriodic.FractionalDelay
- Modelica.Clocked.BooleanSignals.NonPeriodic.UnitDelay

### Clocked.BooleanSignals.Sampler (10 个)

- Modelica.Clocked.BooleanSignals.Sampler.AssignClock
- Modelica.Clocked.BooleanSignals.Sampler.AssignClockVectorized
- Modelica.Clocked.BooleanSignals.Sampler.BackSample
- Modelica.Clocked.BooleanSignals.Sampler.Hold
- Modelica.Clocked.BooleanSignals.Sampler.Sample
- Modelica.Clocked.BooleanSignals.Sampler.SampleClocked
- Modelica.Clocked.BooleanSignals.Sampler.SampleVectorizedAndClocked
- Modelica.Clocked.BooleanSignals.Sampler.ShiftSample
- Modelica.Clocked.BooleanSignals.Sampler.SubSample
- Modelica.Clocked.BooleanSignals.Sampler.SuperSample

### Clocked.BooleanSignals.Sampler.Utilities (3 个)

- Modelica.Clocked.BooleanSignals.Sampler.Utilities.AssignClockToSquareWaveHold
- Modelica.Clocked.BooleanSignals.Sampler.Utilities.AssignClockToTriggerHold
- Modelica.Clocked.BooleanSignals.Sampler.Utilities.UpSample

### Clocked.BooleanSignals.TickBasedSources (2 个)

- Modelica.Clocked.BooleanSignals.TickBasedSources.Pulse
- Modelica.Clocked.BooleanSignals.TickBasedSources.Step

### Clocked.BooleanSignals.TimeBasedSources (2 个)

- Modelica.Clocked.BooleanSignals.TimeBasedSources.Pulse
- Modelica.Clocked.BooleanSignals.TimeBasedSources.Step

### Clocked.ClockSignals.Clocks (3 个)

- Modelica.Clocked.ClockSignals.Clocks.EventClock
- Modelica.Clocked.ClockSignals.Clocks.PeriodicExactClock
- Modelica.Clocked.ClockSignals.Clocks.PeriodicRealClock

### Clocked.ClockSignals.Clocks.Logical (1 个)

- Modelica.Clocked.ClockSignals.Clocks.Logical.PartialLogicalClock

### Clocked.ClockSignals.Clocks.Rotational (2 个)

- Modelica.Clocked.ClockSignals.Clocks.Rotational.FixedRotationalClock
- Modelica.Clocked.ClockSignals.Clocks.Rotational.RotationalClock

### Clocked.ClockSignals.Sampler (3 个)

- Modelica.Clocked.ClockSignals.Sampler.ShiftSample
- Modelica.Clocked.ClockSignals.Sampler.SubSample
- Modelica.Clocked.ClockSignals.Sampler.SuperSample

### Clocked.Examples.CascadeControlledDrive (4 个)

- Modelica.Clocked.Examples.CascadeControlledDrive.AbsoluteClocks
- Modelica.Clocked.Examples.CascadeControlledDrive.Continuous
- Modelica.Clocked.Examples.CascadeControlledDrive.SubClocked
- Modelica.Clocked.Examples.CascadeControlledDrive.SuperSampled

### Clocked.Examples.Elementary.BooleanSignals (16 个)

- Modelica.Clocked.Examples.Elementary.BooleanSignals.AssignClock
- Modelica.Clocked.Examples.Elementary.BooleanSignals.AssignClockVectorized
- Modelica.Clocked.Examples.Elementary.BooleanSignals.BackSample
- Modelica.Clocked.Examples.Elementary.BooleanSignals.Hold
- Modelica.Clocked.Examples.Elementary.BooleanSignals.Sample1
- Modelica.Clocked.Examples.Elementary.BooleanSignals.Sample2
- Modelica.Clocked.Examples.Elementary.BooleanSignals.SampleClocked
- Modelica.Clocked.Examples.Elementary.BooleanSignals.SampleVectorizedAndClocked
- Modelica.Clocked.Examples.Elementary.BooleanSignals.ShiftSample
- Modelica.Clocked.Examples.Elementary.BooleanSignals.SubSample
- Modelica.Clocked.Examples.Elementary.BooleanSignals.SuperSample
- Modelica.Clocked.Examples.Elementary.BooleanSignals.TickBasedPulse
- Modelica.Clocked.Examples.Elementary.BooleanSignals.TickBasedStep
- Modelica.Clocked.Examples.Elementary.BooleanSignals.TimeBasedPulse
- Modelica.Clocked.Examples.Elementary.BooleanSignals.TimeBasedStep
- Modelica.Clocked.Examples.Elementary.BooleanSignals.UpSample

### Clocked.Examples.Elementary.ClockSignals (5 个)

- Modelica.Clocked.Examples.Elementary.ClockSignals.LogicalSample
- Modelica.Clocked.Examples.Elementary.ClockSignals.RotationalSample
- Modelica.Clocked.Examples.Elementary.ClockSignals.ShiftSample
- Modelica.Clocked.Examples.Elementary.ClockSignals.SubSample
- Modelica.Clocked.Examples.Elementary.ClockSignals.SuperSample

### Clocked.Examples.Elementary.IntegerSignals (14 个)

- Modelica.Clocked.Examples.Elementary.IntegerSignals.AssignClock
- Modelica.Clocked.Examples.Elementary.IntegerSignals.AssignClockVectorized
- Modelica.Clocked.Examples.Elementary.IntegerSignals.BackSample
- Modelica.Clocked.Examples.Elementary.IntegerSignals.Hold
- Modelica.Clocked.Examples.Elementary.IntegerSignals.Sample1
- Modelica.Clocked.Examples.Elementary.IntegerSignals.Sample2
- Modelica.Clocked.Examples.Elementary.IntegerSignals.SampleClocked
- Modelica.Clocked.Examples.Elementary.IntegerSignals.SampleVectorizedAndClocked
- Modelica.Clocked.Examples.Elementary.IntegerSignals.ShiftSample
- Modelica.Clocked.Examples.Elementary.IntegerSignals.SubSample
- Modelica.Clocked.Examples.Elementary.IntegerSignals.SuperSample
- Modelica.Clocked.Examples.Elementary.IntegerSignals.TickBasedStep
- Modelica.Clocked.Examples.Elementary.IntegerSignals.TimeBasedStep
- Modelica.Clocked.Examples.Elementary.IntegerSignals.UpSample

### Clocked.Examples.Elementary.RealSignals (29 个)

- Modelica.Clocked.Examples.Elementary.RealSignals.AssignClock
- Modelica.Clocked.Examples.Elementary.RealSignals.AssignClockToSquareWaveHold
- Modelica.Clocked.Examples.Elementary.RealSignals.AssignClockToTriggerHold
- Modelica.Clocked.Examples.Elementary.RealSignals.AssignClockVectorized
- Modelica.Clocked.Examples.Elementary.RealSignals.BackSample
- Modelica.Clocked.Examples.Elementary.RealSignals.FractionalDelay
- Modelica.Clocked.Examples.Elementary.RealSignals.Hold
- Modelica.Clocked.Examples.Elementary.RealSignals.HoldWithDAeffects1
- Modelica.Clocked.Examples.Elementary.RealSignals.HoldWithDAeffects2
- Modelica.Clocked.Examples.Elementary.RealSignals.Sample1
- Modelica.Clocked.Examples.Elementary.RealSignals.Sample2
- Modelica.Clocked.Examples.Elementary.RealSignals.Sample3
- Modelica.Clocked.Examples.Elementary.RealSignals.SampleClocked
- Modelica.Clocked.Examples.Elementary.RealSignals.SampleVectorizedAndClocked
- Modelica.Clocked.Examples.Elementary.RealSignals.SampleWithADeffects
- Modelica.Clocked.Examples.Elementary.RealSignals.ShiftSample
- Modelica.Clocked.Examples.Elementary.RealSignals.SubSample
- Modelica.Clocked.Examples.Elementary.RealSignals.SuperSample
- Modelica.Clocked.Examples.Elementary.RealSignals.SuperSampleInterpolated
- Modelica.Clocked.Examples.Elementary.RealSignals.TickBasedRamp
- Modelica.Clocked.Examples.Elementary.RealSignals.TickBasedSine
- Modelica.Clocked.Examples.Elementary.RealSignals.TickBasedStep
- Modelica.Clocked.Examples.Elementary.RealSignals.TimeBasedRamp
- Modelica.Clocked.Examples.Elementary.RealSignals.TimeBasedSine
- Modelica.Clocked.Examples.Elementary.RealSignals.TimeBasedStep
- Modelica.Clocked.Examples.Elementary.RealSignals.UniformNoise
- Modelica.Clocked.Examples.Elementary.RealSignals.UniformNoiseXorshift64star
- Modelica.Clocked.Examples.Elementary.RealSignals.UpSample1
- Modelica.Clocked.Examples.Elementary.RealSignals.UpSample2

### Clocked.Examples.SimpleControlledDrive (6 个)

- Modelica.Clocked.Examples.SimpleControlledDrive.ClockedWith_AD_DA_Effects
- Modelica.Clocked.Examples.SimpleControlledDrive.ClockedWithDiscreteController
- Modelica.Clocked.Examples.SimpleControlledDrive.ClockedWithDiscreteTextbookController
- Modelica.Clocked.Examples.SimpleControlledDrive.ClockedWithDiscretizedContinuousController
- Modelica.Clocked.Examples.SimpleControlledDrive.Continuous
- Modelica.Clocked.Examples.SimpleControlledDrive.ExactlyClockedWithDiscreteController

### Clocked.Examples.Systems (2 个)

- Modelica.Clocked.Examples.Systems.ControlledMixingUnit
- Modelica.Clocked.Examples.Systems.EngineThrottleControl

### Clocked.Examples.Systems.Utilities.ComponentsMixingUnit (4 个)

- Modelica.Clocked.Examples.Systems.Utilities.ComponentsMixingUnit.CriticalDamping
- Modelica.Clocked.Examples.Systems.Utilities.ComponentsMixingUnit.FilterOrder
- Modelica.Clocked.Examples.Systems.Utilities.ComponentsMixingUnit.MixingUnit
- Modelica.Clocked.Examples.Systems.Utilities.ComponentsMixingUnit.MixingUnitWithContinuousControl

### Clocked.Examples.Systems.Utilities.ComponentsThrottleControl (7 个)

- Modelica.Clocked.Examples.Systems.Utilities.ComponentsThrottleControl.CylinderAirCharge
- Modelica.Clocked.Examples.Systems.Utilities.ComponentsThrottleControl.Engine
- Modelica.Clocked.Examples.Systems.Utilities.ComponentsThrottleControl.InductionToPowerDelay
- Modelica.Clocked.Examples.Systems.Utilities.ComponentsThrottleControl.IntakeManifold
- Modelica.Clocked.Examples.Systems.Utilities.ComponentsThrottleControl.SpeedControl
- Modelica.Clocked.Examples.Systems.Utilities.ComponentsThrottleControl.ThrottleBody
- Modelica.Clocked.Examples.Systems.Utilities.ComponentsThrottleControl.TorqueGeneration

### Clocked.IntegerSignals.NonPeriodic (3 个)

- Modelica.Clocked.IntegerSignals.NonPeriodic.FractionalDelay
- Modelica.Clocked.IntegerSignals.NonPeriodic.IntegerChange
- Modelica.Clocked.IntegerSignals.NonPeriodic.UnitDelay

### Clocked.IntegerSignals.Sampler (10 个)

- Modelica.Clocked.IntegerSignals.Sampler.AssignClock
- Modelica.Clocked.IntegerSignals.Sampler.AssignClockVectorized
- Modelica.Clocked.IntegerSignals.Sampler.BackSample
- Modelica.Clocked.IntegerSignals.Sampler.Hold
- Modelica.Clocked.IntegerSignals.Sampler.Sample
- Modelica.Clocked.IntegerSignals.Sampler.SampleClocked
- Modelica.Clocked.IntegerSignals.Sampler.SampleVectorizedAndClocked
- Modelica.Clocked.IntegerSignals.Sampler.ShiftSample
- Modelica.Clocked.IntegerSignals.Sampler.SubSample
- Modelica.Clocked.IntegerSignals.Sampler.SuperSample

### Clocked.IntegerSignals.Sampler.Utilities (3 个)

- Modelica.Clocked.IntegerSignals.Sampler.Utilities.AssignClockToSquareWaveHold
- Modelica.Clocked.IntegerSignals.Sampler.Utilities.AssignClockToTriggerHold
- Modelica.Clocked.IntegerSignals.Sampler.Utilities.UpSample

### Clocked.IntegerSignals.TickBasedSources (1 个)

- Modelica.Clocked.IntegerSignals.TickBasedSources.Step

### Clocked.IntegerSignals.TimeBasedSources (1 个)

- Modelica.Clocked.IntegerSignals.TimeBasedSources.Step

### Clocked.RealSignals.Interfaces (1 个)

- Modelica.Clocked.RealSignals.Interfaces.PartialSISOSampler

### Clocked.RealSignals.NonPeriodic (3 个)

- Modelica.Clocked.RealSignals.NonPeriodic.FractionalDelay
- Modelica.Clocked.RealSignals.NonPeriodic.PI
- Modelica.Clocked.RealSignals.NonPeriodic.UnitDelay

### Clocked.RealSignals.Periodic (5 个)

- Modelica.Clocked.RealSignals.Periodic.FIRbyCoefficients
- Modelica.Clocked.RealSignals.Periodic.MovingAverage
- Modelica.Clocked.RealSignals.Periodic.PI
- Modelica.Clocked.RealSignals.Periodic.StateSpace
- Modelica.Clocked.RealSignals.Periodic.TransferFunction

### Clocked.RealSignals.Sampler (13 个)

- Modelica.Clocked.RealSignals.Sampler.AssignClock
- Modelica.Clocked.RealSignals.Sampler.AssignClockVectorized
- Modelica.Clocked.RealSignals.Sampler.BackSample
- Modelica.Clocked.RealSignals.Sampler.Hold
- Modelica.Clocked.RealSignals.Sampler.HoldWithDAeffects
- Modelica.Clocked.RealSignals.Sampler.Sample
- Modelica.Clocked.RealSignals.Sampler.SampleClocked
- Modelica.Clocked.RealSignals.Sampler.SampleVectorizedAndClocked
- Modelica.Clocked.RealSignals.Sampler.SampleWithADeffects
- Modelica.Clocked.RealSignals.Sampler.ShiftSample
- Modelica.Clocked.RealSignals.Sampler.SubSample
- Modelica.Clocked.RealSignals.Sampler.SuperSample
- Modelica.Clocked.RealSignals.Sampler.SuperSampleInterpolated

### Clocked.RealSignals.Sampler.Utilities (3 个)

- Modelica.Clocked.RealSignals.Sampler.Utilities.AssignClockToSquareWaveHold
- Modelica.Clocked.RealSignals.Sampler.Utilities.AssignClockToTriggerHold
- Modelica.Clocked.RealSignals.Sampler.Utilities.UpSample

### Clocked.RealSignals.Sampler.Utilities.Internal (5 个)

- Modelica.Clocked.RealSignals.Sampler.Utilities.Internal.ComputationalDelay
- Modelica.Clocked.RealSignals.Sampler.Utilities.Internal.Limiter
- Modelica.Clocked.RealSignals.Sampler.Utilities.Internal.Quantization
- Modelica.Clocked.RealSignals.Sampler.Utilities.Internal.UniformNoise
- Modelica.Clocked.RealSignals.Sampler.Utilities.Internal.UniformNoiseXorshift64star

### Clocked.RealSignals.TickBasedSources (3 个)

- Modelica.Clocked.RealSignals.TickBasedSources.Ramp
- Modelica.Clocked.RealSignals.TickBasedSources.Sine
- Modelica.Clocked.RealSignals.TickBasedSources.Step

### Clocked.RealSignals.TimeBasedSources (3 个)

- Modelica.Clocked.RealSignals.TimeBasedSources.Ramp
- Modelica.Clocked.RealSignals.TimeBasedSources.Sine
- Modelica.Clocked.RealSignals.TimeBasedSources.Step

### Clocked.UsersGuide (1 个)

- Modelica.Clocked.UsersGuide.Clocks

### ComplexBlocks.ComplexMath (26 个)

- Modelica.ComplexBlocks.ComplexMath.Acos
- Modelica.ComplexBlocks.ComplexMath.Add
- Modelica.ComplexBlocks.ComplexMath.Add3
- Modelica.ComplexBlocks.ComplexMath.Asin
- Modelica.ComplexBlocks.ComplexMath.Atan
- Modelica.ComplexBlocks.ComplexMath.Bode
- Modelica.ComplexBlocks.ComplexMath.ComplexToPolar
- Modelica.ComplexBlocks.ComplexMath.ComplexToReal
- Modelica.ComplexBlocks.ComplexMath.Conj
- Modelica.ComplexBlocks.ComplexMath.Cos
- Modelica.ComplexBlocks.ComplexMath.Cosh
- Modelica.ComplexBlocks.ComplexMath.Division
- Modelica.ComplexBlocks.ComplexMath.Exp
- Modelica.ComplexBlocks.ComplexMath.Feedback
- Modelica.ComplexBlocks.ComplexMath.Gain
- Modelica.ComplexBlocks.ComplexMath.Log
- Modelica.ComplexBlocks.ComplexMath.PolarToComplex
- Modelica.ComplexBlocks.ComplexMath.Product
- Modelica.ComplexBlocks.ComplexMath.RealToComplex
- Modelica.ComplexBlocks.ComplexMath.Sin
- Modelica.ComplexBlocks.ComplexMath.Sinh
- Modelica.ComplexBlocks.ComplexMath.Sqrt
- Modelica.ComplexBlocks.ComplexMath.Sum
- Modelica.ComplexBlocks.ComplexMath.Tan
- Modelica.ComplexBlocks.ComplexMath.Tanh
- Modelica.ComplexBlocks.ComplexMath.TransferFunction

### ComplexBlocks.Examples (2 个)

- Modelica.ComplexBlocks.Examples.ShowTransferFunction
- Modelica.ComplexBlocks.Examples.TestConversionBlock

### ComplexBlocks.Routing (4 个)

- Modelica.ComplexBlocks.Routing.ComplexPassThrough
- Modelica.ComplexBlocks.Routing.Extractor
- Modelica.ComplexBlocks.Routing.ExtractSignal
- Modelica.ComplexBlocks.Routing.Replicator

### ComplexBlocks.Sources (4 个)

- Modelica.ComplexBlocks.Sources.ComplexConstant
- Modelica.ComplexBlocks.Sources.ComplexRampPhasor
- Modelica.ComplexBlocks.Sources.ComplexRotatingPhasor
- Modelica.ComplexBlocks.Sources.ComplexStep

### Electrical (144 个)

- Modelica.Electrical.Adder
- Modelica.Electrical.Adder4
- Modelica.Electrical.Additionals
- Modelica.Electrical.And
- Modelica.Electrical.AndGate
- Modelica.Electrical.Basic
- Modelica.Electrical.Basic
- Modelica.Electrical.BJT2
- Modelica.Electrical.BooleanToLogic
- Modelica.Electrical.BUF3S
- Modelica.Electrical.BUF3S
- Modelica.Electrical.BUF3SL
- Modelica.Electrical.BufGate
- Modelica.Electrical.C_Capacitor
- Modelica.Electrical.C_SEMI
- Modelica.Electrical.CascodeCircuit
- Modelica.Electrical.ConditionalSubstrate
- Modelica.Electrical.Converters
- Modelica.Electrical.Counter
- Modelica.Electrical.Counter
- Modelica.Electrical.Counter3
- Modelica.Electrical.Counter3
- Modelica.Electrical.CoupledInductors
- Modelica.Electrical.Delay
- Modelica.Electrical.DFF
- Modelica.Electrical.DFFREG
- Modelica.Electrical.DFFREG
- Modelica.Electrical.DFFREGL
- Modelica.Electrical.DFFREGSRH
- Modelica.Electrical.DFFREGSRH
- Modelica.Electrical.DFFREGSRL
- Modelica.Electrical.Digital
- Modelica.Electrical.DigitalClock
- Modelica.Electrical.Diode
- Modelica.Electrical.DIODE
- Modelica.Electrical.DLATREG
- Modelica.Electrical.DLATREG
- Modelica.Electrical.DLATREGL
- Modelica.Electrical.DLATREGSRH
- Modelica.Electrical.DLATREGSRH
- Modelica.Electrical.DLATREGSRL
- Modelica.Electrical.E_VCV
- Modelica.Electrical.E_VCV_POLY
- Modelica.Electrical.Examples
- Modelica.Electrical.Examples
- Modelica.Electrical.F_CCC
- Modelica.Electrical.F_CCC_POLY
- Modelica.Electrical.FlipFlop
- Modelica.Electrical.FOURBIT
- Modelica.Electrical.FourInverters
- Modelica.Electrical.FullAdder
- Modelica.Electrical.FullAdder
- Modelica.Electrical.G_VCC
- Modelica.Electrical.G_VCC_POLY
- Modelica.Electrical.Gates
- Modelica.Electrical.Graetz
- Modelica.Electrical.Ground
- Modelica.Electrical.H_CCV
- Modelica.Electrical.H_CCV_POLY
- Modelica.Electrical.HalfAdder
- Modelica.Electrical.HalfAdder
- Modelica.Electrical.I_constant
- Modelica.Electrical.I_exp
- Modelica.Electrical.I_pulse
- Modelica.Electrical.I_pwl
- Modelica.Electrical.I_sffm
- Modelica.Electrical.I_sin
- Modelica.Electrical.InertialDelaySensitiveVector
- Modelica.Electrical.Interfaces
- Modelica.Electrical.Interfaces
- Modelica.Electrical.Internal
- Modelica.Electrical.INV3S
- Modelica.Electrical.INV3S
- Modelica.Electrical.INV3SL
- Modelica.Electrical.Inverter
- Modelica.Electrical.InvertersApartRecord
- Modelica.Electrical.InvertersExtendedModel
- Modelica.Electrical.InvGate
- Modelica.Electrical.JFET
- Modelica.Electrical.JKFF
- Modelica.Electrical.K_CoupledInductors
- Modelica.Electrical.L_Inductor
- Modelica.Electrical.LogicToBoolean
- Modelica.Electrical.LogicToReal
- Modelica.Electrical.MemoryBase
- Modelica.Electrical.MOS
- Modelica.Electrical.MOS2
- Modelica.Electrical.Multiplexer
- Modelica.Electrical.Multiplexers
- Modelica.Electrical.MUX2x1
- Modelica.Electrical.MUX2x1
- Modelica.Electrical.MUX4
- Modelica.Electrical.Nand
- Modelica.Electrical.Nand
- Modelica.Electrical.NAND
- Modelica.Electrical.NandGate
- Modelica.Electrical.Nor
- Modelica.Electrical.Nor
- Modelica.Electrical.NorGate
- Modelica.Electrical.Not
- Modelica.Electrical.NRXFER
- Modelica.Electrical.NRXFERGATE
- Modelica.Electrical.NXFER
- Modelica.Electrical.NXFERGATE
- Modelica.Electrical.ONEBIT
- Modelica.Electrical.Or
- Modelica.Electrical.OrGate
- Modelica.Electrical.Oscillator
- Modelica.Electrical.PRXFERGATE
- Modelica.Electrical.Pulse
- Modelica.Electrical.PXFERGATE
- Modelica.Electrical.R_Resistor
- Modelica.Electrical.RAM
- Modelica.Electrical.RealToLogic
- Modelica.Electrical.Registers
- Modelica.Electrical.RS
- Modelica.Electrical.RSFF
- Modelica.Electrical.Sources
- Modelica.Electrical.Sources
- Modelica.Electrical.Spice3
- Modelica.Electrical.Spice3BenchmarkDifferentialPair
- Modelica.Electrical.Spice3BenchmarkFourBitBinaryAdder
- Modelica.Electrical.Spice3BenchmarkMosfetCharacterization
- Modelica.Electrical.Spice3BenchmarkRtlInverter
- Modelica.Electrical.Spicenetlist
- Modelica.Electrical.TransportDelay
- Modelica.Electrical.Tristates
- Modelica.Electrical.TWOBIT
- Modelica.Electrical.TwoPortControlledSources
- Modelica.Electrical.UsersGuide
- Modelica.Electrical.Utilities
- Modelica.Electrical.V_constant
- Modelica.Electrical.V_exp
- Modelica.Electrical.V_pulse
- Modelica.Electrical.V_pwl
- Modelica.Electrical.V_sffm
- Modelica.Electrical.V_sin
- Modelica.Electrical.VectorDelay
- Modelica.Electrical.WiredX
- Modelica.Electrical.WiredX
- Modelica.Electrical.Xnor
- Modelica.Electrical.XnorGate
- Modelica.Electrical.Xor
- Modelica.Electrical.XorGate

### Electrical.Analog.Basic (24 个)

- Modelica.Electrical.Analog.Basic.Capacitor
- Modelica.Electrical.Analog.Basic.CCC
- Modelica.Electrical.Analog.Basic.CCV
- Modelica.Electrical.Analog.Basic.Conductor
- Modelica.Electrical.Analog.Basic.GeneralCurrentToVoltageAdaptor
- Modelica.Electrical.Analog.Basic.GeneralVoltageToCurrentAdaptor
- Modelica.Electrical.Analog.Basic.Ground
- Modelica.Electrical.Analog.Basic.Gyrator
- Modelica.Electrical.Analog.Basic.Inductor
- Modelica.Electrical.Analog.Basic.M_Transformer
- Modelica.Electrical.Analog.Basic.OpAmp
- Modelica.Electrical.Analog.Basic.OpAmpDetailed
- Modelica.Electrical.Analog.Basic.Potentiometer
- Modelica.Electrical.Analog.Basic.Resistor
- Modelica.Electrical.Analog.Basic.RotationalEMF
- Modelica.Electrical.Analog.Basic.SaturatingInductor
- Modelica.Electrical.Analog.Basic.Transformer
- Modelica.Electrical.Analog.Basic.TranslationalEMF
- Modelica.Electrical.Analog.Basic.VariableCapacitor
- Modelica.Electrical.Analog.Basic.VariableConductor
- Modelica.Electrical.Analog.Basic.VariableInductor
- Modelica.Electrical.Analog.Basic.VariableResistor
- Modelica.Electrical.Analog.Basic.VCC
- Modelica.Electrical.Analog.Basic.VCV

### Electrical.Analog.Examples (30 个)

- Modelica.Electrical.Analog.Examples.AD_DA_conversion
- Modelica.Electrical.Analog.Examples.AmplifierWithOpAmpDetailed
- Modelica.Electrical.Analog.Examples.CauerLowPassAnalog
- Modelica.Electrical.Analog.Examples.CauerLowPassOPV
- Modelica.Electrical.Analog.Examples.CauerLowPassSC
- Modelica.Electrical.Analog.Examples.CharacteristicIdealDiodes
- Modelica.Electrical.Analog.Examples.CharacteristicThyristors
- Modelica.Electrical.Analog.Examples.ChuaCircuit
- Modelica.Electrical.Analog.Examples.CompareTransformers
- Modelica.Electrical.Analog.Examples.ControlledSwitchWithArc
- Modelica.Electrical.Analog.Examples.DifferenceAmplifier
- Modelica.Electrical.Analog.Examples.GenerationOfFMUs
- Modelica.Electrical.Analog.Examples.HeatingMOSInverter
- Modelica.Electrical.Analog.Examples.HeatingNPN_NORGate
- Modelica.Electrical.Analog.Examples.HeatingPNP_NORGate
- Modelica.Electrical.Analog.Examples.HeatingRectifier
- Modelica.Electrical.Analog.Examples.IdealTriacCircuit
- Modelica.Electrical.Analog.Examples.InvertingAmp
- Modelica.Electrical.Analog.Examples.NandGate
- Modelica.Electrical.Analog.Examples.OvervoltageProtection
- Modelica.Electrical.Analog.Examples.ParallelResonance
- Modelica.Electrical.Analog.Examples.Rectifier
- Modelica.Electrical.Analog.Examples.Resistor
- Modelica.Electrical.Analog.Examples.ResonanceCircuits
- Modelica.Electrical.Analog.Examples.SeriesResonance
- Modelica.Electrical.Analog.Examples.ShowSaturatingInductor
- Modelica.Electrical.Analog.Examples.ShowVariableResistor
- Modelica.Electrical.Analog.Examples.SimpleTriacCircuit
- Modelica.Electrical.Analog.Examples.SwitchWithArc
- Modelica.Electrical.Analog.Examples.ThyristorBehaviourTest

### Electrical.Analog.Examples.OpAmps (17 个)

- Modelica.Electrical.Analog.Examples.OpAmps.Adder
- Modelica.Electrical.Analog.Examples.OpAmps.Comparator
- Modelica.Electrical.Analog.Examples.OpAmps.ControlCircuit
- Modelica.Electrical.Analog.Examples.OpAmps.DifferentialAmplifier
- Modelica.Electrical.Analog.Examples.OpAmps.Differentiator
- Modelica.Electrical.Analog.Examples.OpAmps.HighPass
- Modelica.Electrical.Analog.Examples.OpAmps.Integrator
- Modelica.Electrical.Analog.Examples.OpAmps.InvertingAmplifier
- Modelica.Electrical.Analog.Examples.OpAmps.InvertingSchmittTrigger
- Modelica.Electrical.Analog.Examples.OpAmps.LCOscillator
- Modelica.Electrical.Analog.Examples.OpAmps.LowPass
- Modelica.Electrical.Analog.Examples.OpAmps.Multivibrator
- Modelica.Electrical.Analog.Examples.OpAmps.NonInvertingAmplifier
- Modelica.Electrical.Analog.Examples.OpAmps.SchmittTrigger
- Modelica.Electrical.Analog.Examples.OpAmps.SignalGenerator
- Modelica.Electrical.Analog.Examples.OpAmps.Subtracter
- Modelica.Electrical.Analog.Examples.OpAmps.VoltageFollower

### Electrical.Analog.Examples.OpAmps.OpAmpCircuits (9 个)

- Modelica.Electrical.Analog.Examples.OpAmps.OpAmpCircuits.Add
- Modelica.Electrical.Analog.Examples.OpAmps.OpAmpCircuits.Buffer
- Modelica.Electrical.Analog.Examples.OpAmps.OpAmpCircuits.Der
- Modelica.Electrical.Analog.Examples.OpAmps.OpAmpCircuits.Derivative
- Modelica.Electrical.Analog.Examples.OpAmps.OpAmpCircuits.Feedback
- Modelica.Electrical.Analog.Examples.OpAmps.OpAmpCircuits.FirstOrder
- Modelica.Electrical.Analog.Examples.OpAmps.OpAmpCircuits.Gain
- Modelica.Electrical.Analog.Examples.OpAmps.OpAmpCircuits.Integrator
- Modelica.Electrical.Analog.Examples.OpAmps.OpAmpCircuits.PI

### Electrical.Analog.Examples.Utilities (11 个)

- Modelica.Electrical.Analog.Examples.Utilities.Conductor
- Modelica.Electrical.Analog.Examples.Utilities.DirectCapacitor
- Modelica.Electrical.Analog.Examples.Utilities.DirectInductor
- Modelica.Electrical.Analog.Examples.Utilities.InverseCapacitor
- Modelica.Electrical.Analog.Examples.Utilities.InverseInductor
- Modelica.Electrical.Analog.Examples.Utilities.Nand
- Modelica.Electrical.Analog.Examples.Utilities.NonlinearResistor
- Modelica.Electrical.Analog.Examples.Utilities.RealSwitch
- Modelica.Electrical.Analog.Examples.Utilities.Resistor
- Modelica.Electrical.Analog.Examples.Utilities.SwitchedCapacitor
- Modelica.Electrical.Analog.Examples.Utilities.Transistor

### Electrical.Analog.Ideal (25 个)

- Modelica.Electrical.Analog.Ideal.AD_Converter
- Modelica.Electrical.Analog.Ideal.CloserWithArc
- Modelica.Electrical.Analog.Ideal.ControlledCloserWithArc
- Modelica.Electrical.Analog.Ideal.ControlledIdealClosingSwitch
- Modelica.Electrical.Analog.Ideal.ControlledIdealIntermediateSwitch
- Modelica.Electrical.Analog.Ideal.ControlledIdealOpeningSwitch
- Modelica.Electrical.Analog.Ideal.ControlledIdealTwoWaySwitch
- Modelica.Electrical.Analog.Ideal.ControlledOpenerWithArc
- Modelica.Electrical.Analog.Ideal.DA_Converter
- Modelica.Electrical.Analog.Ideal.IdealClosingSwitch
- Modelica.Electrical.Analog.Ideal.IdealDiode
- Modelica.Electrical.Analog.Ideal.IdealGTOThyristor
- Modelica.Electrical.Analog.Ideal.IdealIntermediateSwitch
- Modelica.Electrical.Analog.Ideal.IdealizedOpAmpLimited
- Modelica.Electrical.Analog.Ideal.IdealOpAmp
- Modelica.Electrical.Analog.Ideal.IdealOpAmp3Pin
- Modelica.Electrical.Analog.Ideal.IdealOpAmpLimited
- Modelica.Electrical.Analog.Ideal.IdealOpeningSwitch
- Modelica.Electrical.Analog.Ideal.IdealThyristor
- Modelica.Electrical.Analog.Ideal.IdealTransformer
- Modelica.Electrical.Analog.Ideal.IdealTriac
- Modelica.Electrical.Analog.Ideal.IdealTwoWaySwitch
- Modelica.Electrical.Analog.Ideal.Idle
- Modelica.Electrical.Analog.Ideal.OpenerWithArc
- Modelica.Electrical.Analog.Ideal.Short

### Electrical.Analog.Interfaces (11 个)

- Modelica.Electrical.Analog.Interfaces.ConditionalHeatPort
- Modelica.Electrical.Analog.Interfaces.CurrentSource
- Modelica.Electrical.Analog.Interfaces.FourPin
- Modelica.Electrical.Analog.Interfaces.IdealSemiconductor
- Modelica.Electrical.Analog.Interfaces.IdealSwitch
- Modelica.Electrical.Analog.Interfaces.IdealSwitchWithArc
- Modelica.Electrical.Analog.Interfaces.OnePort
- Modelica.Electrical.Analog.Interfaces.PartialConditionalHeatPort
- Modelica.Electrical.Analog.Interfaces.TwoPin
- Modelica.Electrical.Analog.Interfaces.TwoPort
- Modelica.Electrical.Analog.Interfaces.VoltageSource

### Electrical.Analog.Lines (6 个)

- Modelica.Electrical.Analog.Lines.M_OLine
- Modelica.Electrical.Analog.Lines.OLine
- Modelica.Electrical.Analog.Lines.TLine1
- Modelica.Electrical.Analog.Lines.TLine2
- Modelica.Electrical.Analog.Lines.TLine3
- Modelica.Electrical.Analog.Lines.ULine

### Electrical.Analog.Semiconductors (9 个)

- Modelica.Electrical.Analog.Semiconductors.Diode
- Modelica.Electrical.Analog.Semiconductors.Diode2
- Modelica.Electrical.Analog.Semiconductors.NMOS
- Modelica.Electrical.Analog.Semiconductors.NPN
- Modelica.Electrical.Analog.Semiconductors.PMOS
- Modelica.Electrical.Analog.Semiconductors.PNP
- Modelica.Electrical.Analog.Semiconductors.SimpleTriac
- Modelica.Electrical.Analog.Semiconductors.Thyristor
- Modelica.Electrical.Analog.Semiconductors.ZDiode

### Electrical.Analog.Sensors (5 个)

- Modelica.Electrical.Analog.Sensors.CurrentSensor
- Modelica.Electrical.Analog.Sensors.MultiSensor
- Modelica.Electrical.Analog.Sensors.PotentialSensor
- Modelica.Electrical.Analog.Sensors.PowerSensor
- Modelica.Electrical.Analog.Sensors.VoltageSensor

### Electrical.Analog.Sources (9 个)

- Modelica.Electrical.Analog.Sources.ConstantCurrent
- Modelica.Electrical.Analog.Sources.ConstantVoltage
- Modelica.Electrical.Analog.Sources.CosineCurrentVariableFrequencyAndAmplitude
- Modelica.Electrical.Analog.Sources.CosineVoltageVariableFrequencyAndAmplitude
- Modelica.Electrical.Analog.Sources.SignalCurrent
- Modelica.Electrical.Analog.Sources.SignalVoltage
- Modelica.Electrical.Analog.Sources.SineCurrentVariableFrequencyAndAmplitude
- Modelica.Electrical.Analog.Sources.SineVoltageVariableFrequencyAndAmplitude
- Modelica.Electrical.Analog.Sources.SupplyVoltage

### Electrical.Batteries.BaseClasses (3 个)

- Modelica.Electrical.Batteries.BaseClasses.BaseCellStack
- Modelica.Electrical.Batteries.BaseClasses.BaseCellWithSensors
- Modelica.Electrical.Batteries.BaseClasses.BaseStackWithSensors

### Electrical.Batteries.BatteryStacks (3 个)

- Modelica.Electrical.Batteries.BatteryStacks.CellRCStack
- Modelica.Electrical.Batteries.BatteryStacks.CellStack
- Modelica.Electrical.Batteries.BatteryStacks.SuperCap

### Electrical.Batteries.Examples (8 个)

- Modelica.Electrical.Batteries.Examples.BatteryDischargeCharge
- Modelica.Electrical.Batteries.Examples.CCCV_Cell
- Modelica.Electrical.Batteries.Examples.CCCV_CellRC
- Modelica.Electrical.Batteries.Examples.CCCV_Stack
- Modelica.Electrical.Batteries.Examples.CCCV_StackRC
- Modelica.Electrical.Batteries.Examples.CCCVcharging
- Modelica.Electrical.Batteries.Examples.ShowImpedance
- Modelica.Electrical.Batteries.Examples.SuperCapDischargeCharge

### Electrical.Batteries.Utilities (4 个)

- Modelica.Electrical.Batteries.Utilities.BusTranscription
- Modelica.Electrical.Batteries.Utilities.CCCVcharger
- Modelica.Electrical.Batteries.Utilities.Impedance
- Modelica.Electrical.Batteries.Utilities.PulseSeries

### Electrical.Machines.BasicMachines.Components (14 个)

- Modelica.Electrical.Machines.BasicMachines.Components.AirGapDC
- Modelica.Electrical.Machines.BasicMachines.Components.AirGapR
- Modelica.Electrical.Machines.BasicMachines.Components.AirGapS
- Modelica.Electrical.Machines.BasicMachines.Components.CompoundDCExcitation
- Modelica.Electrical.Machines.BasicMachines.Components.DamperCage
- Modelica.Electrical.Machines.BasicMachines.Components.ElectricalExcitation
- Modelica.Electrical.Machines.BasicMachines.Components.IdealCore
- Modelica.Electrical.Machines.BasicMachines.Components.Inductor
- Modelica.Electrical.Machines.BasicMachines.Components.InductorDC
- Modelica.Electrical.Machines.BasicMachines.Components.PartialAirGap
- Modelica.Electrical.Machines.BasicMachines.Components.PartialAirGapDC
- Modelica.Electrical.Machines.BasicMachines.Components.PartialCore
- Modelica.Electrical.Machines.BasicMachines.Components.PermanentMagnet
- Modelica.Electrical.Machines.BasicMachines.Components.SquirrelCage

### Electrical.Machines.BasicMachines.DCMachines (3 个)

- Modelica.Electrical.Machines.BasicMachines.DCMachines.DC_ElectricalExcited
- Modelica.Electrical.Machines.BasicMachines.DCMachines.DC_PermanentMagnet
- Modelica.Electrical.Machines.BasicMachines.DCMachines.DC_SeriesExcited

### Electrical.Machines.BasicMachines.InductionMachines (2 个)

- Modelica.Electrical.Machines.BasicMachines.InductionMachines.IM_SlipRing
- Modelica.Electrical.Machines.BasicMachines.InductionMachines.IM_SquirrelCage

### Electrical.Machines.BasicMachines.SynchronousMachines (3 个)

- Modelica.Electrical.Machines.BasicMachines.SynchronousMachines.SM_ElectricalExcited
- Modelica.Electrical.Machines.BasicMachines.SynchronousMachines.SM_PermanentMagnet
- Modelica.Electrical.Machines.BasicMachines.SynchronousMachines.SM_ReluctanceRotor

### Electrical.Machines.BasicMachines.Transformers.Dd (6 个)

- Modelica.Electrical.Machines.BasicMachines.Transformers.Dd.Dd00
- Modelica.Electrical.Machines.BasicMachines.Transformers.Dd.Dd02
- Modelica.Electrical.Machines.BasicMachines.Transformers.Dd.Dd04
- Modelica.Electrical.Machines.BasicMachines.Transformers.Dd.Dd06
- Modelica.Electrical.Machines.BasicMachines.Transformers.Dd.Dd08
- Modelica.Electrical.Machines.BasicMachines.Transformers.Dd.Dd10

### Electrical.Machines.BasicMachines.Transformers.Dy (6 个)

- Modelica.Electrical.Machines.BasicMachines.Transformers.Dy.Dy01
- Modelica.Electrical.Machines.BasicMachines.Transformers.Dy.Dy03
- Modelica.Electrical.Machines.BasicMachines.Transformers.Dy.Dy05
- Modelica.Electrical.Machines.BasicMachines.Transformers.Dy.Dy07
- Modelica.Electrical.Machines.BasicMachines.Transformers.Dy.Dy09
- Modelica.Electrical.Machines.BasicMachines.Transformers.Dy.Dy11

### Electrical.Machines.BasicMachines.Transformers.Dz (6 个)

- Modelica.Electrical.Machines.BasicMachines.Transformers.Dz.Dz00
- Modelica.Electrical.Machines.BasicMachines.Transformers.Dz.Dz02
- Modelica.Electrical.Machines.BasicMachines.Transformers.Dz.Dz04
- Modelica.Electrical.Machines.BasicMachines.Transformers.Dz.Dz06
- Modelica.Electrical.Machines.BasicMachines.Transformers.Dz.Dz08
- Modelica.Electrical.Machines.BasicMachines.Transformers.Dz.Dz10

### Electrical.Machines.BasicMachines.Transformers.Yd (6 个)

- Modelica.Electrical.Machines.BasicMachines.Transformers.Yd.Yd01
- Modelica.Electrical.Machines.BasicMachines.Transformers.Yd.Yd03
- Modelica.Electrical.Machines.BasicMachines.Transformers.Yd.Yd05
- Modelica.Electrical.Machines.BasicMachines.Transformers.Yd.Yd07
- Modelica.Electrical.Machines.BasicMachines.Transformers.Yd.Yd09
- Modelica.Electrical.Machines.BasicMachines.Transformers.Yd.Yd11

### Electrical.Machines.BasicMachines.Transformers.Yy (6 个)

- Modelica.Electrical.Machines.BasicMachines.Transformers.Yy.Yy00
- Modelica.Electrical.Machines.BasicMachines.Transformers.Yy.Yy02
- Modelica.Electrical.Machines.BasicMachines.Transformers.Yy.Yy04
- Modelica.Electrical.Machines.BasicMachines.Transformers.Yy.Yy06
- Modelica.Electrical.Machines.BasicMachines.Transformers.Yy.Yy08
- Modelica.Electrical.Machines.BasicMachines.Transformers.Yy.Yy10

### Electrical.Machines.BasicMachines.Transformers.Yz (6 个)

- Modelica.Electrical.Machines.BasicMachines.Transformers.Yz.Yz01
- Modelica.Electrical.Machines.BasicMachines.Transformers.Yz.Yz03
- Modelica.Electrical.Machines.BasicMachines.Transformers.Yz.Yz05
- Modelica.Electrical.Machines.BasicMachines.Transformers.Yz.Yz07
- Modelica.Electrical.Machines.BasicMachines.Transformers.Yz.Yz09
- Modelica.Electrical.Machines.BasicMachines.Transformers.Yz.Yz11

### Electrical.Machines.Examples.ControlledDCDrives (3 个)

- Modelica.Electrical.Machines.Examples.ControlledDCDrives.CurrentControlledDCPM
- Modelica.Electrical.Machines.Examples.ControlledDCDrives.PositionControlledDCPM
- Modelica.Electrical.Machines.Examples.ControlledDCDrives.SpeedControlledDCPM

### Electrical.Machines.Examples.ControlledDCDrives.Utilities (6 个)

- Modelica.Electrical.Machines.Examples.ControlledDCDrives.Utilities.Battery
- Modelica.Electrical.Machines.Examples.ControlledDCDrives.Utilities.DcdcInverter
- Modelica.Electrical.Machines.Examples.ControlledDCDrives.Utilities.IdealDcDc
- Modelica.Electrical.Machines.Examples.ControlledDCDrives.Utilities.LimitedPI
- Modelica.Electrical.Machines.Examples.ControlledDCDrives.Utilities.PartialControlledDCPM
- Modelica.Electrical.Machines.Examples.ControlledDCDrives.Utilities.SwitchingDcDc

### Electrical.Machines.Examples.DCMachines (10 个)

- Modelica.Electrical.Machines.Examples.DCMachines.DC_CompareCharacteristics
- Modelica.Electrical.Machines.Examples.DCMachines.DCEE_Start
- Modelica.Electrical.Machines.Examples.DCMachines.DCPM_Cooling
- Modelica.Electrical.Machines.Examples.DCMachines.DCPM_CurrentControlled
- Modelica.Electrical.Machines.Examples.DCMachines.DCPM_QuasiStatic
- Modelica.Electrical.Machines.Examples.DCMachines.DCPM_Start
- Modelica.Electrical.Machines.Examples.DCMachines.DCPM_Temperature
- Modelica.Electrical.Machines.Examples.DCMachines.DCPM_withLosses
- Modelica.Electrical.Machines.Examples.DCMachines.DCSE_SinglePhase
- Modelica.Electrical.Machines.Examples.DCMachines.DCSE_Start

### Electrical.Machines.Examples.InductionMachines (12 个)

- Modelica.Electrical.Machines.Examples.InductionMachines.IMC_Conveyor
- Modelica.Electrical.Machines.Examples.InductionMachines.IMC_DCBraking
- Modelica.Electrical.Machines.Examples.InductionMachines.IMC_DOL
- Modelica.Electrical.Machines.Examples.InductionMachines.IMC_Initialize
- Modelica.Electrical.Machines.Examples.InductionMachines.IMC_Inverter
- Modelica.Electrical.Machines.Examples.InductionMachines.IMC_InverterDrive
- Modelica.Electrical.Machines.Examples.InductionMachines.IMC_Steinmetz
- Modelica.Electrical.Machines.Examples.InductionMachines.IMC_Transformer
- Modelica.Electrical.Machines.Examples.InductionMachines.IMC_withLosses
- Modelica.Electrical.Machines.Examples.InductionMachines.IMC_YD
- Modelica.Electrical.Machines.Examples.InductionMachines.IMC_YDarc
- Modelica.Electrical.Machines.Examples.InductionMachines.IMS_Start

### Electrical.Machines.Examples.SynchronousMachines (12 个)

- Modelica.Electrical.Machines.Examples.SynchronousMachines.SMEE_DOL
- Modelica.Electrical.Machines.Examples.SynchronousMachines.SMEE_Generator
- Modelica.Electrical.Machines.Examples.SynchronousMachines.SMEE_LoadDump
- Modelica.Electrical.Machines.Examples.SynchronousMachines.SMEE_Rectifier
- Modelica.Electrical.Machines.Examples.SynchronousMachines.SMPM_Braking
- Modelica.Electrical.Machines.Examples.SynchronousMachines.SMPM_CurrentSource
- Modelica.Electrical.Machines.Examples.SynchronousMachines.SMPM_Inverter
- Modelica.Electrical.Machines.Examples.SynchronousMachines.SMPM_NoLoad
- Modelica.Electrical.Machines.Examples.SynchronousMachines.SMPM_ResistiveBraking
- Modelica.Electrical.Machines.Examples.SynchronousMachines.SMPM_VoltageSource
- Modelica.Electrical.Machines.Examples.SynchronousMachines.SMR_DOL
- Modelica.Electrical.Machines.Examples.SynchronousMachines.SMR_Inverter

### Electrical.Machines.Examples.Transformers (4 个)

- Modelica.Electrical.Machines.Examples.Transformers.AsymmetricalLoad
- Modelica.Electrical.Machines.Examples.Transformers.Rectifier12pulse
- Modelica.Electrical.Machines.Examples.Transformers.Rectifier6pulse
- Modelica.Electrical.Machines.Examples.Transformers.TransformerTestbench

### Electrical.Machines.Interfaces (5 个)

- Modelica.Electrical.Machines.Interfaces.FlangeSupport
- Modelica.Electrical.Machines.Interfaces.PartialBasicDCMachine
- Modelica.Electrical.Machines.Interfaces.PartialBasicInductionMachine
- Modelica.Electrical.Machines.Interfaces.PartialBasicMachine
- Modelica.Electrical.Machines.Interfaces.PartialBasicTransformer

### Electrical.Machines.Interfaces.DCMachines (1 个)

- Modelica.Electrical.Machines.Interfaces.DCMachines.PartialThermalAmbientDCMachines

### Electrical.Machines.Interfaces.InductionMachines (1 个)

- Modelica.Electrical.Machines.Interfaces.InductionMachines.PartialThermalAmbientInductionMachines

### Electrical.Machines.Losses (1 个)

- Modelica.Electrical.Machines.Losses.Friction

### Electrical.Machines.Losses.DCMachines (3 个)

- Modelica.Electrical.Machines.Losses.DCMachines.Brush
- Modelica.Electrical.Machines.Losses.DCMachines.Core
- Modelica.Electrical.Machines.Losses.DCMachines.StrayLoad

### Electrical.Machines.Losses.InductionMachines (4 个)

- Modelica.Electrical.Machines.Losses.InductionMachines.Brush
- Modelica.Electrical.Machines.Losses.InductionMachines.Core
- Modelica.Electrical.Machines.Losses.InductionMachines.PermanentMagnetLosses
- Modelica.Electrical.Machines.Losses.InductionMachines.StrayLoad

### Electrical.Machines.Sensors (7 个)

- Modelica.Electrical.Machines.Sensors.CurrentQuasiRMSSensor
- Modelica.Electrical.Machines.Sensors.ElectricalPowerSensor
- Modelica.Electrical.Machines.Sensors.HallSensor
- Modelica.Electrical.Machines.Sensors.MechanicalPowerSensor
- Modelica.Electrical.Machines.Sensors.RotorDisplacementAngle
- Modelica.Electrical.Machines.Sensors.SinCosResolver
- Modelica.Electrical.Machines.Sensors.VoltageQuasiRMSSensor

### Electrical.Machines.SpacePhasors.Blocks (7 个)

- Modelica.Electrical.Machines.SpacePhasors.Blocks.FromPolar
- Modelica.Electrical.Machines.SpacePhasors.Blocks.FromSpacePhasor
- Modelica.Electrical.Machines.SpacePhasors.Blocks.LessThreshold
- Modelica.Electrical.Machines.SpacePhasors.Blocks.QuasiRMS
- Modelica.Electrical.Machines.SpacePhasors.Blocks.Rotator
- Modelica.Electrical.Machines.SpacePhasors.Blocks.ToPolar
- Modelica.Electrical.Machines.SpacePhasors.Blocks.ToSpacePhasor

### Electrical.Machines.SpacePhasors.Components (2 个)

- Modelica.Electrical.Machines.SpacePhasors.Components.Rotator
- Modelica.Electrical.Machines.SpacePhasors.Components.SpacePhasor

### Electrical.Machines.Thermal (1 个)

- Modelica.Electrical.Machines.Thermal.ThermalAmbientTransformer

### Electrical.Machines.Thermal.DCMachines (4 个)

- Modelica.Electrical.Machines.Thermal.DCMachines.ThermalAmbientDCCE
- Modelica.Electrical.Machines.Thermal.DCMachines.ThermalAmbientDCEE
- Modelica.Electrical.Machines.Thermal.DCMachines.ThermalAmbientDCPM
- Modelica.Electrical.Machines.Thermal.DCMachines.ThermalAmbientDCSE

### Electrical.Machines.Thermal.InductionMachines (2 个)

- Modelica.Electrical.Machines.Thermal.InductionMachines.ThermalAmbientIMC
- Modelica.Electrical.Machines.Thermal.InductionMachines.ThermalAmbientIMS

### Electrical.Machines.Thermal.SynchronousMachines (3 个)

- Modelica.Electrical.Machines.Thermal.SynchronousMachines.ThermalAmbientSMEE
- Modelica.Electrical.Machines.Thermal.SynchronousMachines.ThermalAmbientSMPM
- Modelica.Electrical.Machines.Thermal.SynchronousMachines.ThermalAmbientSMR

### Electrical.Machines.Utilities (12 个)

- Modelica.Electrical.Machines.Utilities.DQCurrentController
- Modelica.Electrical.Machines.Utilities.DQToThreePhase
- Modelica.Electrical.Machines.Utilities.FromDQ
- Modelica.Electrical.Machines.Utilities.MultiTerminalBox
- Modelica.Electrical.Machines.Utilities.RampedRheostat
- Modelica.Electrical.Machines.Utilities.SinCosEvaluation
- Modelica.Electrical.Machines.Utilities.SwitchedRheostat
- Modelica.Electrical.Machines.Utilities.SwitchYD
- Modelica.Electrical.Machines.Utilities.SwitchYDwithArc
- Modelica.Electrical.Machines.Utilities.TerminalBox
- Modelica.Electrical.Machines.Utilities.ToDQ
- Modelica.Electrical.Machines.Utilities.VfController

### Electrical.Polyphase.Basic (22 个)

- Modelica.Electrical.Polyphase.Basic.Capacitor
- Modelica.Electrical.Polyphase.Basic.Conductor
- Modelica.Electrical.Polyphase.Basic.Delta
- Modelica.Electrical.Polyphase.Basic.Inductor
- Modelica.Electrical.Polyphase.Basic.MultiDelta
- Modelica.Electrical.Polyphase.Basic.MultiStar
- Modelica.Electrical.Polyphase.Basic.MultiStarResistance
- Modelica.Electrical.Polyphase.Basic.MutualInductor
- Modelica.Electrical.Polyphase.Basic.PlugToPin_n
- Modelica.Electrical.Polyphase.Basic.PlugToPin_p
- Modelica.Electrical.Polyphase.Basic.PlugToPins_n
- Modelica.Electrical.Polyphase.Basic.PlugToPins_p
- Modelica.Electrical.Polyphase.Basic.Resistor
- Modelica.Electrical.Polyphase.Basic.SaturatingInductor
- Modelica.Electrical.Polyphase.Basic.SplitToSubsystems
- Modelica.Electrical.Polyphase.Basic.Star
- Modelica.Electrical.Polyphase.Basic.Transformer
- Modelica.Electrical.Polyphase.Basic.VariableCapacitor
- Modelica.Electrical.Polyphase.Basic.VariableConductor
- Modelica.Electrical.Polyphase.Basic.VariableInductor
- Modelica.Electrical.Polyphase.Basic.VariableResistor
- Modelica.Electrical.Polyphase.Basic.ZeroInductor

### Electrical.Polyphase.Blocks (1 个)

- Modelica.Electrical.Polyphase.Blocks.QuasiRMS

### Electrical.Polyphase.Examples (5 个)

- Modelica.Electrical.Polyphase.Examples.PolyphaseRectifier
- Modelica.Electrical.Polyphase.Examples.Rectifier
- Modelica.Electrical.Polyphase.Examples.TestSensors
- Modelica.Electrical.Polyphase.Examples.TransformerYD
- Modelica.Electrical.Polyphase.Examples.TransformerYY

### Electrical.Polyphase.Examples.Utilities (2 个)

- Modelica.Electrical.Polyphase.Examples.Utilities.AnalysatorAC
- Modelica.Electrical.Polyphase.Examples.Utilities.AnalysatorDC

### Electrical.Polyphase.Ideal (12 个)

- Modelica.Electrical.Polyphase.Ideal.CloserWithArc
- Modelica.Electrical.Polyphase.Ideal.IdealClosingSwitch
- Modelica.Electrical.Polyphase.Ideal.IdealCommutingSwitch
- Modelica.Electrical.Polyphase.Ideal.IdealDiode
- Modelica.Electrical.Polyphase.Ideal.IdealGTOThyristor
- Modelica.Electrical.Polyphase.Ideal.IdealIntermediateSwitch
- Modelica.Electrical.Polyphase.Ideal.IdealOpeningSwitch
- Modelica.Electrical.Polyphase.Ideal.IdealThyristor
- Modelica.Electrical.Polyphase.Ideal.IdealTransformer
- Modelica.Electrical.Polyphase.Ideal.Idle
- Modelica.Electrical.Polyphase.Ideal.OpenerWithArc
- Modelica.Electrical.Polyphase.Ideal.Short

### Electrical.Polyphase.Interfaces (4 个)

- Modelica.Electrical.Polyphase.Interfaces.FourPlug
- Modelica.Electrical.Polyphase.Interfaces.OnePort
- Modelica.Electrical.Polyphase.Interfaces.TwoPlug
- Modelica.Electrical.Polyphase.Interfaces.TwoPort

### Electrical.Polyphase.Sensors (9 个)

- Modelica.Electrical.Polyphase.Sensors.AronSensor
- Modelica.Electrical.Polyphase.Sensors.CurrentQuasiRMSSensor
- Modelica.Electrical.Polyphase.Sensors.CurrentSensor
- Modelica.Electrical.Polyphase.Sensors.MultiSensor
- Modelica.Electrical.Polyphase.Sensors.PotentialSensor
- Modelica.Electrical.Polyphase.Sensors.PowerSensor
- Modelica.Electrical.Polyphase.Sensors.ReactivePowerSensor
- Modelica.Electrical.Polyphase.Sensors.VoltageQuasiRMSSensor
- Modelica.Electrical.Polyphase.Sensors.VoltageSensor

### Electrical.Polyphase.Sources (8 个)

- Modelica.Electrical.Polyphase.Sources.ConstantCurrent
- Modelica.Electrical.Polyphase.Sources.ConstantVoltage
- Modelica.Electrical.Polyphase.Sources.CosineCurrent
- Modelica.Electrical.Polyphase.Sources.CosineVoltage
- Modelica.Electrical.Polyphase.Sources.SignalCurrent
- Modelica.Electrical.Polyphase.Sources.SignalVoltage
- Modelica.Electrical.Polyphase.Sources.SineCurrent
- Modelica.Electrical.Polyphase.Sources.SineVoltage

### Electrical.PowerConverters.ACAC (2 个)

- Modelica.Electrical.PowerConverters.ACAC.PolyphaseTriac
- Modelica.Electrical.PowerConverters.ACAC.SinglePhaseTriac

### Electrical.PowerConverters.ACAC.Control (2 个)

- Modelica.Electrical.PowerConverters.ACAC.Control.SoftStartControl
- Modelica.Electrical.PowerConverters.ACAC.Control.VoltageToAngle

### Electrical.PowerConverters.ACDC (12 个)

- Modelica.Electrical.PowerConverters.ACDC.DiodeBridge2mPulse
- Modelica.Electrical.PowerConverters.ACDC.DiodeBridge2Pulse
- Modelica.Electrical.PowerConverters.ACDC.DiodeCenterTap2mPulse
- Modelica.Electrical.PowerConverters.ACDC.DiodeCenterTap2Pulse
- Modelica.Electrical.PowerConverters.ACDC.DiodeCenterTapmPulse
- Modelica.Electrical.PowerConverters.ACDC.HalfControlledBridge2mPulse
- Modelica.Electrical.PowerConverters.ACDC.HalfControlledBridge2Pulse
- Modelica.Electrical.PowerConverters.ACDC.ThyristorBridge2mPulse
- Modelica.Electrical.PowerConverters.ACDC.ThyristorBridge2Pulse
- Modelica.Electrical.PowerConverters.ACDC.ThyristorCenterTap2mPulse
- Modelica.Electrical.PowerConverters.ACDC.ThyristorCenterTap2Pulse
- Modelica.Electrical.PowerConverters.ACDC.ThyristorCenterTapmPulse

### Electrical.PowerConverters.ACDC.Control (5 个)

- Modelica.Electrical.PowerConverters.ACDC.Control.Filter
- Modelica.Electrical.PowerConverters.ACDC.Control.Signal2mPulse
- Modelica.Electrical.PowerConverters.ACDC.Control.VoltageBridge2mPulse
- Modelica.Electrical.PowerConverters.ACDC.Control.VoltageBridge2Pulse
- Modelica.Electrical.PowerConverters.ACDC.Control.VoltageCenterTap2mPulse

### Electrical.PowerConverters.DCAC (2 个)

- Modelica.Electrical.PowerConverters.DCAC.Polyphase2Level
- Modelica.Electrical.PowerConverters.DCAC.SinglePhase2Level

### Electrical.PowerConverters.DCAC.Control (3 个)

- Modelica.Electrical.PowerConverters.DCAC.Control.IntersectivePWM
- Modelica.Electrical.PowerConverters.DCAC.Control.PWM
- Modelica.Electrical.PowerConverters.DCAC.Control.SVPWM

### Electrical.PowerConverters.DCDC (3 个)

- Modelica.Electrical.PowerConverters.DCDC.ChopperStepDown
- Modelica.Electrical.PowerConverters.DCDC.ChopperStepUp
- Modelica.Electrical.PowerConverters.DCDC.HBridge

### Electrical.PowerConverters.DCDC.Control (2 个)

- Modelica.Electrical.PowerConverters.DCDC.Control.SignalPWM
- Modelica.Electrical.PowerConverters.DCDC.Control.Voltage2DutyCycle

### Electrical.PowerConverters.Enable (1 个)

- Modelica.Electrical.PowerConverters.Enable.EnableLogic

### Electrical.PowerConverters.Examples.ACAC (3 个)

- Modelica.Electrical.PowerConverters.Examples.ACAC.Dimmer_R
- Modelica.Electrical.PowerConverters.Examples.ACAC.Dimmer_RL
- Modelica.Electrical.PowerConverters.Examples.ACAC.SoftStarter

### Electrical.PowerConverters.Examples.ACAC.ExampleTemplates (1 个)

- Modelica.Electrical.PowerConverters.Examples.ACAC.ExampleTemplates.Dimmer

### Electrical.PowerConverters.Examples.ACDC.ExampleTemplates (6 个)

- Modelica.Electrical.PowerConverters.Examples.ACDC.ExampleTemplates.Thyristor1Pulse
- Modelica.Electrical.PowerConverters.Examples.ACDC.ExampleTemplates.ThyristorBridge2mPulse
- Modelica.Electrical.PowerConverters.Examples.ACDC.ExampleTemplates.ThyristorBridge2Pulse
- Modelica.Electrical.PowerConverters.Examples.ACDC.ExampleTemplates.ThyristorCenterTap2mPulse
- Modelica.Electrical.PowerConverters.Examples.ACDC.ExampleTemplates.ThyristorCenterTap2Pulse
- Modelica.Electrical.PowerConverters.Examples.ACDC.ExampleTemplates.ThyristorCenterTapmPulse

### Electrical.PowerConverters.Examples.ACDC.Rectifier1Pulse (2 个)

- Modelica.Electrical.PowerConverters.Examples.ACDC.Rectifier1Pulse.Thyristor1Pulse_R
- Modelica.Electrical.PowerConverters.Examples.ACDC.Rectifier1Pulse.Thyristor1Pulse_R_Characteristic

### Electrical.PowerConverters.Examples.ACDC.RectifierBridge2Pulse (7 个)

- Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2Pulse.DiodeBridge2Pulse
- Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2Pulse.HalfControlledBridge2Pulse
- Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2Pulse.ThyristorBridge2Pulse_DC_Drive
- Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2Pulse.ThyristorBridge2Pulse_R
- Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2Pulse.ThyristorBridge2Pulse_RL
- Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2Pulse.ThyristorBridge2Pulse_RLV
- Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2Pulse.ThyristorBridge2Pulse_RLV_Characteristic

### Electrical.PowerConverters.Examples.ACDC.RectifierBridge2mPulse (7 个)

- Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2mPulse.DiodeBridge2mPulse
- Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2mPulse.HalfControlledBridge2mPulse
- Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2mPulse.ThyristorBridge2mPulse_DC_Drive
- Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2mPulse.ThyristorBridge2mPulse_R
- Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2mPulse.ThyristorBridge2mPulse_RL
- Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2mPulse.ThyristorBridge2mPulse_RLV
- Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierBridge2mPulse.ThyristorBridge2mPulse_RLV_Characteristic

### Electrical.PowerConverters.Examples.ACDC.RectifierCenterTap2Pulse (5 个)

- Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierCenterTap2Pulse.DiodeCenterTap2Pulse
- Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierCenterTap2Pulse.ThyristorCenterTap2Pulse_R
- Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierCenterTap2Pulse.ThyristorCenterTap2Pulse_RL
- Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierCenterTap2Pulse.ThyristorCenterTap2Pulse_RLV
- Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierCenterTap2Pulse.ThyristorCenterTap2Pulse_RLV_Characteristic

### Electrical.PowerConverters.Examples.ACDC.RectifierCenterTap2mPulse (5 个)

- Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierCenterTap2mPulse.DiodeCenterTap2mPulse
- Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierCenterTap2mPulse.ThyristorCenterTap2mPulse_R
- Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierCenterTap2mPulse.ThyristorCenterTap2mPulse_RL
- Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierCenterTap2mPulse.ThyristorCenterTap2mPulse_RLV
- Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierCenterTap2mPulse.ThyristorCenterTap2mPulse_RLV_Characteristic

### Electrical.PowerConverters.Examples.ACDC.RectifierCenterTapmPulse (5 个)

- Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierCenterTapmPulse.DiodeCenterTapmPulse
- Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierCenterTapmPulse.ThyristorCenterTapmPulse_R
- Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierCenterTapmPulse.ThyristorCenterTapmPulse_RL
- Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierCenterTapmPulse.ThyristorCenterTapmPulse_RLV
- Modelica.Electrical.PowerConverters.Examples.ACDC.RectifierCenterTapmPulse.ThyristorCenterTapmPulse_RLV_Characteristic

### Electrical.PowerConverters.Examples.DCAC.ExampleTemplates (1 个)

- Modelica.Electrical.PowerConverters.Examples.DCAC.ExampleTemplates.SinglePhaseTwoLevel

### Electrical.PowerConverters.Examples.DCAC.PolyphaseTwoLevel (3 个)

- Modelica.Electrical.PowerConverters.Examples.DCAC.PolyphaseTwoLevel.PolyphaseTwoLevel_R
- Modelica.Electrical.PowerConverters.Examples.DCAC.PolyphaseTwoLevel.PolyphaseTwoLevel_RL
- Modelica.Electrical.PowerConverters.Examples.DCAC.PolyphaseTwoLevel.ThreePhaseTwoLevel_PWM

### Electrical.PowerConverters.Examples.DCAC.SinglePhaseTwoLevel (2 个)

- Modelica.Electrical.PowerConverters.Examples.DCAC.SinglePhaseTwoLevel.SinglePhaseTwoLevel_R
- Modelica.Electrical.PowerConverters.Examples.DCAC.SinglePhaseTwoLevel.SinglePhaseTwoLevel_RL

### Electrical.PowerConverters.Examples.DCDC.ChopperStepDown (2 个)

- Modelica.Electrical.PowerConverters.Examples.DCDC.ChopperStepDown.ChopperStepDown_R
- Modelica.Electrical.PowerConverters.Examples.DCDC.ChopperStepDown.ChopperStepDown_RL

### Electrical.PowerConverters.Examples.DCDC.ChopperStepUp (1 个)

- Modelica.Electrical.PowerConverters.Examples.DCDC.ChopperStepUp.ChopperStepUp_R

### Electrical.PowerConverters.Examples.DCDC.ExampleTemplates (3 个)

- Modelica.Electrical.PowerConverters.Examples.DCDC.ExampleTemplates.ChopperStepDown
- Modelica.Electrical.PowerConverters.Examples.DCDC.ExampleTemplates.ChopperStepUp
- Modelica.Electrical.PowerConverters.Examples.DCDC.ExampleTemplates.HBridge

### Electrical.PowerConverters.Examples.DCDC.HBridge (3 个)

- Modelica.Electrical.PowerConverters.Examples.DCDC.HBridge.HBridge_DC_Drive
- Modelica.Electrical.PowerConverters.Examples.DCDC.HBridge.HBridge_R
- Modelica.Electrical.PowerConverters.Examples.DCDC.HBridge.HBridge_RL

### Electrical.PowerConverters.Interfaces.Enable (5 个)

- Modelica.Electrical.PowerConverters.Interfaces.Enable.Enable
- Modelica.Electrical.PowerConverters.Interfaces.Enable.Enable1
- Modelica.Electrical.PowerConverters.Interfaces.Enable.Enable1m
- Modelica.Electrical.PowerConverters.Interfaces.Enable.Enable2
- Modelica.Electrical.PowerConverters.Interfaces.Enable.Enable2m

### Electrical.QuasiStatic.Machines.BasicMachines.Components (2 个)

- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Components.IdealCore
- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Components.PartialCore

### Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Dd (6 个)

- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Dd.Dd00
- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Dd.Dd02
- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Dd.Dd04
- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Dd.Dd06
- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Dd.Dd08
- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Dd.Dd10

### Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Dy (6 个)

- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Dy.Dy01
- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Dy.Dy03
- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Dy.Dy05
- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Dy.Dy07
- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Dy.Dy09
- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Dy.Dy11

### Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Dz (6 个)

- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Dz.Dz00
- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Dz.Dz02
- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Dz.Dz04
- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Dz.Dz06
- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Dz.Dz08
- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Dz.Dz10

### Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Yd (6 个)

- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Yd.Yd01
- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Yd.Yd03
- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Yd.Yd05
- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Yd.Yd07
- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Yd.Yd09
- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Yd.Yd11

### Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Yy (6 个)

- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Yy.Yy00
- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Yy.Yy02
- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Yy.Yy04
- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Yy.Yy06
- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Yy.Yy08
- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Yy.Yy10

### Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Yz (6 个)

- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Yz.Yz01
- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Yz.Yz03
- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Yz.Yz05
- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Yz.Yz07
- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Yz.Yz09
- Modelica.Electrical.QuasiStatic.Machines.BasicMachines.Transformers.Yz.Yz11

### Electrical.QuasiStatic.Machines.Examples (1 个)

- Modelica.Electrical.QuasiStatic.Machines.Examples.TransformerTestbench

### Electrical.QuasiStatic.Machines.Interfaces (1 个)

- Modelica.Electrical.QuasiStatic.Machines.Interfaces.PartialBasicTransformer

### Electrical.QuasiStatic.Machines.SpacePhasors.Blocks (2 个)

- Modelica.Electrical.QuasiStatic.Machines.SpacePhasors.Blocks.FromSpacePhasor
- Modelica.Electrical.QuasiStatic.Machines.SpacePhasors.Blocks.ToSpacePhasor

### Electrical.QuasiStatic.Polyphase.Basic (22 个)

- Modelica.Electrical.QuasiStatic.Polyphase.Basic.Admittance
- Modelica.Electrical.QuasiStatic.Polyphase.Basic.Capacitor
- Modelica.Electrical.QuasiStatic.Polyphase.Basic.Conductor
- Modelica.Electrical.QuasiStatic.Polyphase.Basic.Delta
- Modelica.Electrical.QuasiStatic.Polyphase.Basic.Impedance
- Modelica.Electrical.QuasiStatic.Polyphase.Basic.Inductor
- Modelica.Electrical.QuasiStatic.Polyphase.Basic.MultiDelta
- Modelica.Electrical.QuasiStatic.Polyphase.Basic.MultiStar
- Modelica.Electrical.QuasiStatic.Polyphase.Basic.MultiStarResistance
- Modelica.Electrical.QuasiStatic.Polyphase.Basic.MutualInductor
- Modelica.Electrical.QuasiStatic.Polyphase.Basic.PlugToPin_n
- Modelica.Electrical.QuasiStatic.Polyphase.Basic.PlugToPin_p
- Modelica.Electrical.QuasiStatic.Polyphase.Basic.PlugToPins_n
- Modelica.Electrical.QuasiStatic.Polyphase.Basic.PlugToPins_p
- Modelica.Electrical.QuasiStatic.Polyphase.Basic.Resistor
- Modelica.Electrical.QuasiStatic.Polyphase.Basic.Star
- Modelica.Electrical.QuasiStatic.Polyphase.Basic.VariableAdmittance
- Modelica.Electrical.QuasiStatic.Polyphase.Basic.VariableCapacitor
- Modelica.Electrical.QuasiStatic.Polyphase.Basic.VariableConductor
- Modelica.Electrical.QuasiStatic.Polyphase.Basic.VariableImpedance
- Modelica.Electrical.QuasiStatic.Polyphase.Basic.VariableInductor
- Modelica.Electrical.QuasiStatic.Polyphase.Basic.VariableResistor

### Electrical.QuasiStatic.Polyphase.Blocks (6 个)

- Modelica.Electrical.QuasiStatic.Polyphase.Blocks.FromSpacePhasor
- Modelica.Electrical.QuasiStatic.Polyphase.Blocks.FromSymmetricalComponents
- Modelica.Electrical.QuasiStatic.Polyphase.Blocks.QuasiRMS
- Modelica.Electrical.QuasiStatic.Polyphase.Blocks.SingleToPolyphase
- Modelica.Electrical.QuasiStatic.Polyphase.Blocks.SymmetricalComponents
- Modelica.Electrical.QuasiStatic.Polyphase.Blocks.ToSpacePhasor

### Electrical.QuasiStatic.Polyphase.Examples (4 个)

- Modelica.Electrical.QuasiStatic.Polyphase.Examples.BalancingDelta
- Modelica.Electrical.QuasiStatic.Polyphase.Examples.BalancingStar
- Modelica.Electrical.QuasiStatic.Polyphase.Examples.TestSensors
- Modelica.Electrical.QuasiStatic.Polyphase.Examples.UnsymmetricalLoad

### Electrical.QuasiStatic.Polyphase.Ideal (6 个)

- Modelica.Electrical.QuasiStatic.Polyphase.Ideal.IdealClosingSwitch
- Modelica.Electrical.QuasiStatic.Polyphase.Ideal.IdealCommutingSwitch
- Modelica.Electrical.QuasiStatic.Polyphase.Ideal.IdealIntermediateSwitch
- Modelica.Electrical.QuasiStatic.Polyphase.Ideal.IdealOpeningSwitch
- Modelica.Electrical.QuasiStatic.Polyphase.Ideal.Idle
- Modelica.Electrical.QuasiStatic.Polyphase.Ideal.Short

### Electrical.QuasiStatic.Polyphase.Interfaces (6 个)

- Modelica.Electrical.QuasiStatic.Polyphase.Interfaces.AbsoluteSensor
- Modelica.Electrical.QuasiStatic.Polyphase.Interfaces.OnePort
- Modelica.Electrical.QuasiStatic.Polyphase.Interfaces.ReferenceSource
- Modelica.Electrical.QuasiStatic.Polyphase.Interfaces.Source
- Modelica.Electrical.QuasiStatic.Polyphase.Interfaces.TwoPlug
- Modelica.Electrical.QuasiStatic.Polyphase.Interfaces.TwoPlugElementary

### Electrical.QuasiStatic.Polyphase.Sensors (11 个)

- Modelica.Electrical.QuasiStatic.Polyphase.Sensors.AronSensor
- Modelica.Electrical.QuasiStatic.Polyphase.Sensors.CurrentQuasiRMSSensor
- Modelica.Electrical.QuasiStatic.Polyphase.Sensors.CurrentSensor
- Modelica.Electrical.QuasiStatic.Polyphase.Sensors.FrequencySensor
- Modelica.Electrical.QuasiStatic.Polyphase.Sensors.MultiSensor
- Modelica.Electrical.QuasiStatic.Polyphase.Sensors.PotentialSensor
- Modelica.Electrical.QuasiStatic.Polyphase.Sensors.PowerSensor
- Modelica.Electrical.QuasiStatic.Polyphase.Sensors.ReactivePowerSensor
- Modelica.Electrical.QuasiStatic.Polyphase.Sensors.ReferenceSensor
- Modelica.Electrical.QuasiStatic.Polyphase.Sensors.VoltageQuasiRMSSensor
- Modelica.Electrical.QuasiStatic.Polyphase.Sensors.VoltageSensor

### Electrical.QuasiStatic.Polyphase.Sources (8 个)

- Modelica.Electrical.QuasiStatic.Polyphase.Sources.CurrentSource
- Modelica.Electrical.QuasiStatic.Polyphase.Sources.FrequencySweepCurrentSource
- Modelica.Electrical.QuasiStatic.Polyphase.Sources.FrequencySweepVoltageSource
- Modelica.Electrical.QuasiStatic.Polyphase.Sources.ReferenceCurrentSource
- Modelica.Electrical.QuasiStatic.Polyphase.Sources.ReferenceVoltageSource
- Modelica.Electrical.QuasiStatic.Polyphase.Sources.VariableCurrentSource
- Modelica.Electrical.QuasiStatic.Polyphase.Sources.VariableVoltageSource
- Modelica.Electrical.QuasiStatic.Polyphase.Sources.VoltageSource

### Electrical.QuasiStatic.SinglePhase.Basic (13 个)

- Modelica.Electrical.QuasiStatic.SinglePhase.Basic.Admittance
- Modelica.Electrical.QuasiStatic.SinglePhase.Basic.Capacitor
- Modelica.Electrical.QuasiStatic.SinglePhase.Basic.Conductor
- Modelica.Electrical.QuasiStatic.SinglePhase.Basic.Ground
- Modelica.Electrical.QuasiStatic.SinglePhase.Basic.Impedance
- Modelica.Electrical.QuasiStatic.SinglePhase.Basic.Inductor
- Modelica.Electrical.QuasiStatic.SinglePhase.Basic.Resistor
- Modelica.Electrical.QuasiStatic.SinglePhase.Basic.VariableAdmittance
- Modelica.Electrical.QuasiStatic.SinglePhase.Basic.VariableCapacitor
- Modelica.Electrical.QuasiStatic.SinglePhase.Basic.VariableConductor
- Modelica.Electrical.QuasiStatic.SinglePhase.Basic.VariableImpedance
- Modelica.Electrical.QuasiStatic.SinglePhase.Basic.VariableInductor
- Modelica.Electrical.QuasiStatic.SinglePhase.Basic.VariableResistor

### Electrical.QuasiStatic.SinglePhase.Examples (5 个)

- Modelica.Electrical.QuasiStatic.SinglePhase.Examples.ParallelResonance
- Modelica.Electrical.QuasiStatic.SinglePhase.Examples.Rectifier
- Modelica.Electrical.QuasiStatic.SinglePhase.Examples.SeriesBode
- Modelica.Electrical.QuasiStatic.SinglePhase.Examples.SeriesResonance
- Modelica.Electrical.QuasiStatic.SinglePhase.Examples.Transformer

### Electrical.QuasiStatic.SinglePhase.Ideal (7 个)

- Modelica.Electrical.QuasiStatic.SinglePhase.Ideal.IdealClosingSwitch
- Modelica.Electrical.QuasiStatic.SinglePhase.Ideal.IdealCommutingSwitch
- Modelica.Electrical.QuasiStatic.SinglePhase.Ideal.IdealIntermediateSwitch
- Modelica.Electrical.QuasiStatic.SinglePhase.Ideal.IdealOpeningSwitch
- Modelica.Electrical.QuasiStatic.SinglePhase.Ideal.IdealTransformer
- Modelica.Electrical.QuasiStatic.SinglePhase.Ideal.Idle
- Modelica.Electrical.QuasiStatic.SinglePhase.Ideal.Short

### Electrical.QuasiStatic.SinglePhase.Interfaces (6 个)

- Modelica.Electrical.QuasiStatic.SinglePhase.Interfaces.AbsoluteSensor
- Modelica.Electrical.QuasiStatic.SinglePhase.Interfaces.OnePort
- Modelica.Electrical.QuasiStatic.SinglePhase.Interfaces.RelativeSensorElementary
- Modelica.Electrical.QuasiStatic.SinglePhase.Interfaces.Source
- Modelica.Electrical.QuasiStatic.SinglePhase.Interfaces.TwoPin
- Modelica.Electrical.QuasiStatic.SinglePhase.Interfaces.TwoPinElementary

### Electrical.QuasiStatic.SinglePhase.Sensors (7 个)

- Modelica.Electrical.QuasiStatic.SinglePhase.Sensors.CurrentSensor
- Modelica.Electrical.QuasiStatic.SinglePhase.Sensors.FrequencySensor
- Modelica.Electrical.QuasiStatic.SinglePhase.Sensors.MultiSensor
- Modelica.Electrical.QuasiStatic.SinglePhase.Sensors.PotentialSensor
- Modelica.Electrical.QuasiStatic.SinglePhase.Sensors.PowerSensor
- Modelica.Electrical.QuasiStatic.SinglePhase.Sensors.ReferenceSensor
- Modelica.Electrical.QuasiStatic.SinglePhase.Sensors.VoltageSensor

### Electrical.QuasiStatic.SinglePhase.Sources (6 个)

- Modelica.Electrical.QuasiStatic.SinglePhase.Sources.CurrentSource
- Modelica.Electrical.QuasiStatic.SinglePhase.Sources.FrequencySweepCurrentSource
- Modelica.Electrical.QuasiStatic.SinglePhase.Sources.FrequencySweepVoltageSource
- Modelica.Electrical.QuasiStatic.SinglePhase.Sources.VariableCurrentSource
- Modelica.Electrical.QuasiStatic.SinglePhase.Sources.VariableVoltageSource
- Modelica.Electrical.QuasiStatic.SinglePhase.Sources.VoltageSource

### Electrical.QuasiStatic.SinglePhase.Utilities (2 个)

- Modelica.Electrical.QuasiStatic.SinglePhase.Utilities.GraetzRectifier
- Modelica.Electrical.QuasiStatic.SinglePhase.Utilities.IdealACDCConverter

### Electrical.QuasiStatic.UsersGuide.Overview (2 个)

- Modelica.Electrical.QuasiStatic.UsersGuide.Overview.Introduction
- Modelica.Electrical.QuasiStatic.UsersGuide.Overview.Power

### Fluid (128 个)

- Modelica.BalanceEquations
- Modelica.ComponentDefinition
- Modelica.Fluid
- Modelica.Fluid.BaseClasses
- Modelica.Fluid.BaseClasses
- Modelica.Fluid.BaseClasses
- Modelica.Fluid.BaseClasses
- Modelica.Fluid.BaseClasses
- Modelica.Fluid.BaseClasses
- Modelica.Fluid.BaseClasses
- Modelica.Fluid.BaseModel
- Modelica.Fluid.BaseModelNonconstantCrossSectionArea
- Modelica.Fluid.Bend
- Modelica.Fluid.Bends
- Modelica.Fluid.Boundary_ph
- Modelica.Fluid.Boundary_pT
- Modelica.Fluid.ClosedVolume
- Modelica.Fluid.ConstantFlowHeatTransfer
- Modelica.Fluid.ConstantHeatTransfer
- Modelica.Fluid.ControlledPump
- Modelica.Fluid.CurvedBend
- Modelica.Fluid.Density
- Modelica.Fluid.DensityTwoPort
- Modelica.Fluid.Detailed
- Modelica.Fluid.DetailedPipeFlow
- Modelica.Fluid.Dissipation
- Modelica.Fluid.DynamicPipe
- Modelica.Fluid.EdgedBend
- Modelica.Fluid.Fittings
- Modelica.Fluid.FixedBoundary
- Modelica.Fluid.FlowModels
- Modelica.Fluid.General
- Modelica.Fluid.General
- Modelica.Fluid.GenericResistances
- Modelica.Fluid.GettingStarted
- Modelica.Fluid.HeatTransfer
- Modelica.Fluid.HeatTransfer
- Modelica.Fluid.IdealFlowHeatTransfer
- Modelica.Fluid.IdealHeatTransfer
- Modelica.Fluid.Interfaces
- Modelica.Fluid.Laminar
- Modelica.Fluid.LocalPipeFlowHeatTransfer
- Modelica.Fluid.LossFactorData
- Modelica.Fluid.Machines
- Modelica.Fluid.MassFlowRate
- Modelica.Fluid.MassFlowSource_h
- Modelica.Fluid.MassFlowSource_T
- Modelica.Fluid.MassFractions
- Modelica.Fluid.MassFractionsTwoPort
- Modelica.Fluid.MinLimiter
- Modelica.Fluid.MultiPort
- Modelica.Fluid.NominalLaminarFlow
- Modelica.Fluid.NominalTurbulentPipeFlow
- Modelica.Fluid.OpenTank
- Modelica.Fluid.Orifice
- Modelica.Fluid.Orifices
- Modelica.Fluid.PartialAbsoluteSensor
- Modelica.Fluid.PartialDistributedFlow
- Modelica.Fluid.PartialDistributedVolume
- Modelica.Fluid.PartialFlowSensor
- Modelica.Fluid.PartialFlowSource
- Modelica.Fluid.PartialGenericPipeFlow
- Modelica.Fluid.PartialHeatTransfer
- Modelica.Fluid.PartialLumpedFlow
- Modelica.Fluid.PartialLumpedVessel
- Modelica.Fluid.PartialLumpedVolume
- Modelica.Fluid.PartialPipeFlowHeatTransfer
- Modelica.Fluid.PartialPressureLoss
- Modelica.Fluid.PartialPump
- Modelica.Fluid.PartialRelativeSensor
- Modelica.Fluid.PartialSource
- Modelica.Fluid.PartialStaggeredFlowModel
- Modelica.Fluid.PartialStraightPipe
- Modelica.Fluid.PartialTwoPortFlow
- Modelica.Fluid.PartialTwoPortTransport
- Modelica.Fluid.PartialValve
- Modelica.Fluid.Pipes
- Modelica.Fluid.PrescribedPump
- Modelica.Fluid.Pressure
- Modelica.Fluid.PressureLoss
- Modelica.Fluid.Pump
- Modelica.Fluid.QuadraticTurbulent
- Modelica.Fluid.Records
- Modelica.Fluid.RelativePressure
- Modelica.Fluid.RelativeTemperature
- Modelica.Fluid.Sensors
- Modelica.Fluid.SharedDocumentation
- Modelica.Fluid.SimpleGenericOrifice
- Modelica.Fluid.Sources
- Modelica.Fluid.SpecificEnthalpy
- Modelica.Fluid.SpecificEnthalpyTwoPort
- Modelica.Fluid.SpecificEntropy
- Modelica.Fluid.SpecificEntropyTwoPort
- Modelica.Fluid.StaticPipe
- Modelica.Fluid.StraightPipe
- Modelica.Fluid.SweptVolume
- Modelica.Fluid.System
- Modelica.Fluid.TeeJunctionIdeal
- Modelica.Fluid.TeeJunctionVolume
- Modelica.Fluid.Temperature
- Modelica.Fluid.TemperatureTwoPort
- Modelica.Fluid.ThickEdgedOrifice
- Modelica.Fluid.TraceSubstances
- Modelica.Fluid.TraceSubstancesTwoPort
- Modelica.Fluid.TurbulentPipeFlow
- Modelica.Fluid.TwoPhaseFlow
- Modelica.Fluid.Types
- Modelica.Fluid.UsersGuide
- Modelica.Fluid.Utilities
- Modelica.Fluid.Utilities
- Modelica.Fluid.ValveCompressible
- Modelica.Fluid.ValveDiscrete
- Modelica.Fluid.ValveDiscreteRamp
- Modelica.Fluid.ValveIncompressible
- Modelica.Fluid.ValveLinear
- Modelica.Fluid.Valves
- Modelica.Fluid.ValveVaporizing
- Modelica.Fluid.Vessels
- Modelica.Fluid.VolumeFlowRate
- Modelica.Fluid.VolumeFlowRate
- Modelica.Fluid.WallFriction
- Modelica.FluidConnectors
- Modelica.RegularizingCharacteristics
- Modelica.ReleaseNotes
- Modelica.UpstreamDiscretization
- Modelica.UsersGuide
- Modelica.ValveCharacteristics
- Modelica.WallFriction

### Fluid.Examples (44 个)

- Modelica.Fluid.Examples.AST_BatchPlant
- Modelica.Fluid.Examples.BaseClasses
- Modelica.Fluid.Examples.BaseClasses
- Modelica.Fluid.Examples.BaseClasses
- Modelica.Fluid.Examples.BasicHX
- Modelica.Fluid.Examples.BatchPlant_StandardWater
- Modelica.Fluid.Examples.BranchingDynamicPipes
- Modelica.Fluid.Examples.ControlledTanks
- Modelica.Fluid.Examples.ControlledTankSystem
- Modelica.Fluid.Examples.Controller
- Modelica.Fluid.Examples.DrumBoiler
- Modelica.Fluid.Examples.DrumBoiler
- Modelica.Fluid.Examples.EmptyTanks
- Modelica.Fluid.Examples.EquilibriumDrumBoiler
- Modelica.Fluid.Examples.Explanatory
- Modelica.Fluid.Examples.HeatExchanger
- Modelica.Fluid.Examples.HeatExchangerSimulation
- Modelica.Fluid.Examples.HeatingSystem
- Modelica.Fluid.Examples.IncompressibleFluidNetwork
- Modelica.Fluid.Examples.InnerTank
- Modelica.Fluid.Examples.InverseParameterization
- Modelica.Fluid.Examples.MeasuringTemperature
- Modelica.Fluid.Examples.MomentumBalanceFittings
- Modelica.Fluid.Examples.NonCircularPipes
- Modelica.Fluid.Examples.NormalOperation
- Modelica.Fluid.Examples.OneTank
- Modelica.Fluid.Examples.PumpingSystem
- Modelica.Fluid.Examples.RoomCO2
- Modelica.Fluid.Examples.RoomCO2WithControls
- Modelica.Fluid.Examples.TankController
- Modelica.Fluid.Examples.Tanks
- Modelica.Fluid.Examples.TanksWithEmptyingPipe1
- Modelica.Fluid.Examples.TanksWithEmptyingPipe2
- Modelica.Fluid.Examples.TanksWithOverflow
- Modelica.Fluid.Examples.TankWith3InletOutletArraysWithEvaporatorCondensor
- Modelica.Fluid.Examples.TankWithEmptyingPipe1
- Modelica.Fluid.Examples.TankWithEmptyingPipe2
- Modelica.Fluid.Examples.TankWithTopPorts
- Modelica.Fluid.Examples.Test
- Modelica.Fluid.Examples.ThreeTanks
- Modelica.Fluid.Examples.TraceSubstances
- Modelica.Fluid.Examples.TwoTanks
- Modelica.Fluid.Examples.Utilities
- Modelica.Fluid.Examples.WallConstProps

### Magnetic.FluxTubes.BaseClasses (5 个)

- Modelica.Magnetic.FluxTubes.BaseClasses.FixedShape
- Modelica.Magnetic.FluxTubes.BaseClasses.Force
- Modelica.Magnetic.FluxTubes.BaseClasses.GenericHysteresis
- Modelica.Magnetic.FluxTubes.BaseClasses.GenericHysteresisTellinen
- Modelica.Magnetic.FluxTubes.BaseClasses.Leakage

### Magnetic.FluxTubes.Basic (12 个)

- Modelica.Magnetic.FluxTubes.Basic.ConstantPermeance
- Modelica.Magnetic.FluxTubes.Basic.ConstantReluctance
- Modelica.Magnetic.FluxTubes.Basic.Crossing
- Modelica.Magnetic.FluxTubes.Basic.EddyCurrent
- Modelica.Magnetic.FluxTubes.Basic.ElectroMagneticConverter
- Modelica.Magnetic.FluxTubes.Basic.ElectroMagneticConverterWithLeakageInductance
- Modelica.Magnetic.FluxTubes.Basic.Ground
- Modelica.Magnetic.FluxTubes.Basic.Idle
- Modelica.Magnetic.FluxTubes.Basic.LeakageWithCoefficient
- Modelica.Magnetic.FluxTubes.Basic.Short
- Modelica.Magnetic.FluxTubes.Basic.VariablePermeance
- Modelica.Magnetic.FluxTubes.Basic.VariableReluctance

### Magnetic.FluxTubes.Examples.BasicExamples (4 个)

- Modelica.Magnetic.FluxTubes.Examples.BasicExamples.QuadraticCoreAirgap
- Modelica.Magnetic.FluxTubes.Examples.BasicExamples.SaturatedInductor
- Modelica.Magnetic.FluxTubes.Examples.BasicExamples.ToroidalCoreAirgap
- Modelica.Magnetic.FluxTubes.Examples.BasicExamples.ToroidalCoreQuadraticCrossSection

### Magnetic.FluxTubes.Examples.Hysteresis (5 个)

- Modelica.Magnetic.FluxTubes.Examples.Hysteresis.HysteresisModelComparison
- Modelica.Magnetic.FluxTubes.Examples.Hysteresis.InductorWithHysteresis
- Modelica.Magnetic.FluxTubes.Examples.Hysteresis.SinglePhaseTransformerWithHysteresis1
- Modelica.Magnetic.FluxTubes.Examples.Hysteresis.SinglePhaseTransformerWithHysteresis2
- Modelica.Magnetic.FluxTubes.Examples.Hysteresis.ThreePhaseTransformerWithRectifier

### Magnetic.FluxTubes.Examples.Hysteresis.Components (2 个)

- Modelica.Magnetic.FluxTubes.Examples.Hysteresis.Components.Transformer1PhaseWithHysteresis
- Modelica.Magnetic.FluxTubes.Examples.Hysteresis.Components.Transformer3PhaseYyWithHysteresis

### Magnetic.FluxTubes.Examples.MovingCoilActuator (3 个)

- Modelica.Magnetic.FluxTubes.Examples.MovingCoilActuator.ArmatureStroke
- Modelica.Magnetic.FluxTubes.Examples.MovingCoilActuator.ForceCurrentBehaviour
- Modelica.Magnetic.FluxTubes.Examples.MovingCoilActuator.ForceStrokeBehaviour

### Magnetic.FluxTubes.Examples.MovingCoilActuator.Components (2 个)

- Modelica.Magnetic.FluxTubes.Examples.MovingCoilActuator.Components.ConstantActuator
- Modelica.Magnetic.FluxTubes.Examples.MovingCoilActuator.Components.PermeanceActuator

### Magnetic.FluxTubes.Examples.SolenoidActuator (2 个)

- Modelica.Magnetic.FluxTubes.Examples.SolenoidActuator.ComparisonPullInStroke
- Modelica.Magnetic.FluxTubes.Examples.SolenoidActuator.ComparisonQuasiStatic

### Magnetic.FluxTubes.Examples.SolenoidActuator.Components (2 个)

- Modelica.Magnetic.FluxTubes.Examples.SolenoidActuator.Components.AdvancedSolenoid
- Modelica.Magnetic.FluxTubes.Examples.SolenoidActuator.Components.SimpleSolenoid

### Magnetic.FluxTubes.Examples.Utilities (1 个)

- Modelica.Magnetic.FluxTubes.Examples.Utilities.TranslatoryArmatureAndStopper

### Magnetic.FluxTubes.Interfaces (3 个)

- Modelica.Magnetic.FluxTubes.Interfaces.ConditionalHeatPort
- Modelica.Magnetic.FluxTubes.Interfaces.TwoPort
- Modelica.Magnetic.FluxTubes.Interfaces.TwoPortExtended

### Magnetic.FluxTubes.Sensors (2 个)

- Modelica.Magnetic.FluxTubes.Sensors.MagneticFluxSensor
- Modelica.Magnetic.FluxTubes.Sensors.MagneticPotentialDifferenceSensor

### Magnetic.FluxTubes.Shapes.FixedShape (6 个)

- Modelica.Magnetic.FluxTubes.Shapes.FixedShape.Cuboid
- Modelica.Magnetic.FluxTubes.Shapes.FixedShape.GenericFluxTube
- Modelica.Magnetic.FluxTubes.Shapes.FixedShape.HollowCylinderAxialFlux
- Modelica.Magnetic.FluxTubes.Shapes.FixedShape.HollowCylinderCircumferentialFlux
- Modelica.Magnetic.FluxTubes.Shapes.FixedShape.HollowCylinderRadialFlux
- Modelica.Magnetic.FluxTubes.Shapes.FixedShape.Toroid

### Magnetic.FluxTubes.Shapes.Force (6 个)

- Modelica.Magnetic.FluxTubes.Shapes.Force
- Modelica.Magnetic.FluxTubes.Shapes.Force.CuboidOrthogonalFlux
- Modelica.Magnetic.FluxTubes.Shapes.Force.CuboidParallelFlux
- Modelica.Magnetic.FluxTubes.Shapes.Force.HollowCylinderAxialFlux
- Modelica.Magnetic.FluxTubes.Shapes.Force.HollowCylinderRadialFlux
- Modelica.Magnetic.FluxTubes.Shapes.Force.LeakageAroundPoles

### Magnetic.FluxTubes.Shapes.HysteresisAndMagnets (7 个)

- Modelica.Magnetic.FluxTubes.Shapes.HysteresisAndMagnets.GenericHystPreisachEverett
- Modelica.Magnetic.FluxTubes.Shapes.HysteresisAndMagnets.GenericHystTellinenEverett
- Modelica.Magnetic.FluxTubes.Shapes.HysteresisAndMagnets.GenericHystTellinenHard
- Modelica.Magnetic.FluxTubes.Shapes.HysteresisAndMagnets.GenericHystTellinenPermanentMagnet
- Modelica.Magnetic.FluxTubes.Shapes.HysteresisAndMagnets.GenericHystTellinenSoft
- Modelica.Magnetic.FluxTubes.Shapes.HysteresisAndMagnets.GenericHystTellinenTable
- Modelica.Magnetic.FluxTubes.Shapes.HysteresisAndMagnets.GenericLinearPermanentMagnet

### Magnetic.FluxTubes.Shapes.Leakage (9 个)

- Modelica.Magnetic.FluxTubes.Shapes.Leakage.CoaxCylindersEndFaces
- Modelica.Magnetic.FluxTubes.Shapes.Leakage.EighthOfHollowSphere
- Modelica.Magnetic.FluxTubes.Shapes.Leakage.EighthOfSphere
- Modelica.Magnetic.FluxTubes.Shapes.Leakage.HalfCylinder
- Modelica.Magnetic.FluxTubes.Shapes.Leakage.HalfHollowCylinder
- Modelica.Magnetic.FluxTubes.Shapes.Leakage.QuarterCylinder
- Modelica.Magnetic.FluxTubes.Shapes.Leakage.QuarterHollowCylinder
- Modelica.Magnetic.FluxTubes.Shapes.Leakage.QuarterHollowSphere
- Modelica.Magnetic.FluxTubes.Shapes.Leakage.QuarterSphere

### Magnetic.FluxTubes.Sources (4 个)

- Modelica.Magnetic.FluxTubes.Sources.ConstantMagneticFlux
- Modelica.Magnetic.FluxTubes.Sources.ConstantMagneticPotentialDifference
- Modelica.Magnetic.FluxTubes.Sources.SignalMagneticFlux
- Modelica.Magnetic.FluxTubes.Sources.SignalMagneticPotentialDifference

### Magnetic.FluxTubes.UsersGuide (2 个)

- Modelica.Magnetic.FluxTubes.UsersGuide.FluxTubeConcept
- Modelica.Magnetic.FluxTubes.UsersGuide.ReluctanceForceCalculation

### Magnetic.FluxTubes.UsersGuide.Hysteresis.StaticHysteresis (1 个)

- Modelica.Magnetic.FluxTubes.UsersGuide.Hysteresis.StaticHysteresis.Preisach

### Magnetic.FundamentalWave.BaseClasses (1 个)

- Modelica.Magnetic.FundamentalWave.BaseClasses.Machine

### Magnetic.FundamentalWave.BasicMachines.Components (5 个)

- Modelica.Magnetic.FundamentalWave.BasicMachines.Components.RotorSaliencyAirGap
- Modelica.Magnetic.FundamentalWave.BasicMachines.Components.SaliencyCageWinding
- Modelica.Magnetic.FundamentalWave.BasicMachines.Components.SinglePhaseWinding
- Modelica.Magnetic.FundamentalWave.BasicMachines.Components.SymmetricPolyphaseCageWinding
- Modelica.Magnetic.FundamentalWave.BasicMachines.Components.SymmetricPolyphaseWinding

### Magnetic.FundamentalWave.BasicMachines.InductionMachines (2 个)

- Modelica.Magnetic.FundamentalWave.BasicMachines.InductionMachines.IM_SlipRing
- Modelica.Magnetic.FundamentalWave.BasicMachines.InductionMachines.IM_SquirrelCage

### Magnetic.FundamentalWave.BasicMachines.SynchronousMachines (3 个)

- Modelica.Magnetic.FundamentalWave.BasicMachines.SynchronousMachines.SM_ElectricalExcited
- Modelica.Magnetic.FundamentalWave.BasicMachines.SynchronousMachines.SM_PermanentMagnet
- Modelica.Magnetic.FundamentalWave.BasicMachines.SynchronousMachines.SM_ReluctanceRotor

### Magnetic.FundamentalWave.Components (9 个)

- Modelica.Magnetic.FundamentalWave.Components.Crossing
- Modelica.Magnetic.FundamentalWave.Components.EddyCurrent
- Modelica.Magnetic.FundamentalWave.Components.Ground
- Modelica.Magnetic.FundamentalWave.Components.Idle
- Modelica.Magnetic.FundamentalWave.Components.Permeance
- Modelica.Magnetic.FundamentalWave.Components.PolyphaseElectroMagneticConverter
- Modelica.Magnetic.FundamentalWave.Components.Reluctance
- Modelica.Magnetic.FundamentalWave.Components.Short
- Modelica.Magnetic.FundamentalWave.Components.SinglePhaseElectroMagneticConverter

### Magnetic.FundamentalWave.Examples.BasicMachines.InductionMachines (9 个)

- Modelica.Magnetic.FundamentalWave.Examples.BasicMachines.InductionMachines.IMC_Conveyor
- Modelica.Magnetic.FundamentalWave.Examples.BasicMachines.InductionMachines.IMC_DOL
- Modelica.Magnetic.FundamentalWave.Examples.BasicMachines.InductionMachines.IMC_Initialize
- Modelica.Magnetic.FundamentalWave.Examples.BasicMachines.InductionMachines.IMC_Inverter
- Modelica.Magnetic.FundamentalWave.Examples.BasicMachines.InductionMachines.IMC_Steinmetz
- Modelica.Magnetic.FundamentalWave.Examples.BasicMachines.InductionMachines.IMC_Transformer
- Modelica.Magnetic.FundamentalWave.Examples.BasicMachines.InductionMachines.IMC_withLosses
- Modelica.Magnetic.FundamentalWave.Examples.BasicMachines.InductionMachines.IMC_YD
- Modelica.Magnetic.FundamentalWave.Examples.BasicMachines.InductionMachines.IMS_Start

### Magnetic.FundamentalWave.Examples.BasicMachines.InductionMachines.ComparisonPolyphase (2 个)

- Modelica.Magnetic.FundamentalWave.Examples.BasicMachines.InductionMachines.ComparisonPolyphase.IMC_DOL_Polyphase
- Modelica.Magnetic.FundamentalWave.Examples.BasicMachines.InductionMachines.ComparisonPolyphase.IMS_Start_Polyphase

### Magnetic.FundamentalWave.Examples.BasicMachines.SynchronousMachines (9 个)

- Modelica.Magnetic.FundamentalWave.Examples.BasicMachines.SynchronousMachines.SMEE_DOL
- Modelica.Magnetic.FundamentalWave.Examples.BasicMachines.SynchronousMachines.SMEE_Generator
- Modelica.Magnetic.FundamentalWave.Examples.BasicMachines.SynchronousMachines.SMEE_LoadDump
- Modelica.Magnetic.FundamentalWave.Examples.BasicMachines.SynchronousMachines.SMEE_Rectifier
- Modelica.Magnetic.FundamentalWave.Examples.BasicMachines.SynchronousMachines.SMPM_Braking
- Modelica.Magnetic.FundamentalWave.Examples.BasicMachines.SynchronousMachines.SMPM_CurrentSource
- Modelica.Magnetic.FundamentalWave.Examples.BasicMachines.SynchronousMachines.SMPM_Inverter
- Modelica.Magnetic.FundamentalWave.Examples.BasicMachines.SynchronousMachines.SMPM_VoltageSource
- Modelica.Magnetic.FundamentalWave.Examples.BasicMachines.SynchronousMachines.SMR_Inverter

### Magnetic.FundamentalWave.Examples.BasicMachines.SynchronousMachines.ComparisonPolyphase (3 个)

- Modelica.Magnetic.FundamentalWave.Examples.BasicMachines.SynchronousMachines.ComparisonPolyphase.SMEE_Generator_Polyphase
- Modelica.Magnetic.FundamentalWave.Examples.BasicMachines.SynchronousMachines.ComparisonPolyphase.SMPM_Inverter_Polyphase
- Modelica.Magnetic.FundamentalWave.Examples.BasicMachines.SynchronousMachines.ComparisonPolyphase.SMR_Inverter_Polyphase

### Magnetic.FundamentalWave.Examples.Components (3 个)

- Modelica.Magnetic.FundamentalWave.Examples.Components.EddyCurrentLosses
- Modelica.Magnetic.FundamentalWave.Examples.Components.PolyphaseInductance
- Modelica.Magnetic.FundamentalWave.Examples.Components.SinglePhaseInductance

### Magnetic.FundamentalWave.Interfaces (4 个)

- Modelica.Magnetic.FundamentalWave.Interfaces.NegativePortInterface
- Modelica.Magnetic.FundamentalWave.Interfaces.PositivePortInterface
- Modelica.Magnetic.FundamentalWave.Interfaces.StateSelector
- Modelica.Magnetic.FundamentalWave.Interfaces.TwoPort

### Magnetic.FundamentalWave.Sensors (3 个)

- Modelica.Magnetic.FundamentalWave.Sensors.MagneticFluxSensor
- Modelica.Magnetic.FundamentalWave.Sensors.MagneticPotentialDifferenceSensor
- Modelica.Magnetic.FundamentalWave.Sensors.MagneticPotentialSensor

### Magnetic.FundamentalWave.Sources (4 个)

- Modelica.Magnetic.FundamentalWave.Sources.ConstantFlux
- Modelica.Magnetic.FundamentalWave.Sources.ConstantMagneticPotentialDifference
- Modelica.Magnetic.FundamentalWave.Sources.SignalFlux
- Modelica.Magnetic.FundamentalWave.Sources.SignalMagneticPotentialDifference

### Magnetic.QuasiStatic.FluxTubes.BaseClasses (2 个)

- Modelica.Magnetic.QuasiStatic.FluxTubes.BaseClasses.FixedShape
- Modelica.Magnetic.QuasiStatic.FluxTubes.BaseClasses.Leakage

### Magnetic.QuasiStatic.FluxTubes.Basic (11 个)

- Modelica.Magnetic.QuasiStatic.FluxTubes.Basic.ConstantPermeance
- Modelica.Magnetic.QuasiStatic.FluxTubes.Basic.ConstantReluctance
- Modelica.Magnetic.QuasiStatic.FluxTubes.Basic.Crossing
- Modelica.Magnetic.QuasiStatic.FluxTubes.Basic.EddyCurrent
- Modelica.Magnetic.QuasiStatic.FluxTubes.Basic.ElectroMagneticConverter
- Modelica.Magnetic.QuasiStatic.FluxTubes.Basic.Ground
- Modelica.Magnetic.QuasiStatic.FluxTubes.Basic.Idle
- Modelica.Magnetic.QuasiStatic.FluxTubes.Basic.LeakageWithCoefficient
- Modelica.Magnetic.QuasiStatic.FluxTubes.Basic.Short
- Modelica.Magnetic.QuasiStatic.FluxTubes.Basic.VariablePermeance
- Modelica.Magnetic.QuasiStatic.FluxTubes.Basic.VariableReluctance

### Magnetic.QuasiStatic.FluxTubes.Examples (2 个)

- Modelica.Magnetic.QuasiStatic.FluxTubes.Examples.LinearInductor
- Modelica.Magnetic.QuasiStatic.FluxTubes.Examples.NonLinearInductor

### Magnetic.QuasiStatic.FluxTubes.Examples.BasicExamples (3 个)

- Modelica.Magnetic.QuasiStatic.FluxTubes.Examples.BasicExamples.QuadraticCoreAirgap
- Modelica.Magnetic.QuasiStatic.FluxTubes.Examples.BasicExamples.ToroidalCoreAirgap
- Modelica.Magnetic.QuasiStatic.FluxTubes.Examples.BasicExamples.ToroidalCoreQuadraticCrossSection

### Magnetic.QuasiStatic.FluxTubes.Examples.FixedShapes (2 个)

- Modelica.Magnetic.QuasiStatic.FluxTubes.Examples.FixedShapes.CuboidSections
- Modelica.Magnetic.QuasiStatic.FluxTubes.Examples.FixedShapes.CylinderSections

### Magnetic.QuasiStatic.FluxTubes.Examples.Leakage (2 个)

- Modelica.Magnetic.QuasiStatic.FluxTubes.Examples.Leakage.CylinderLeakage
- Modelica.Magnetic.QuasiStatic.FluxTubes.Examples.Leakage.GeneralLeakage

### Magnetic.QuasiStatic.FluxTubes.Interfaces (4 个)

- Modelica.Magnetic.QuasiStatic.FluxTubes.Interfaces.AbsoluteSensor
- Modelica.Magnetic.QuasiStatic.FluxTubes.Interfaces.Source
- Modelica.Magnetic.QuasiStatic.FluxTubes.Interfaces.TwoPort
- Modelica.Magnetic.QuasiStatic.FluxTubes.Interfaces.TwoPortElementary

### Magnetic.QuasiStatic.FluxTubes.Sensors (5 个)

- Modelica.Magnetic.QuasiStatic.FluxTubes.Sensors.FrequencySensor
- Modelica.Magnetic.QuasiStatic.FluxTubes.Sensors.MagneticFluxSensor
- Modelica.Magnetic.QuasiStatic.FluxTubes.Sensors.MagneticPotentialDifferenceSensor
- Modelica.Magnetic.QuasiStatic.FluxTubes.Sensors.MagneticPotentialSensor
- Modelica.Magnetic.QuasiStatic.FluxTubes.Sensors.ReferenceSensor

### Magnetic.QuasiStatic.FluxTubes.Sensors.Transient (2 个)

- Modelica.Magnetic.QuasiStatic.FluxTubes.Sensors.Transient.FundamentalWavePermabilitySensor
- Modelica.Magnetic.QuasiStatic.FluxTubes.Sensors.Transient.Permeability

### Magnetic.QuasiStatic.FluxTubes.Shapes.FixedShape (6 个)

- Modelica.Magnetic.QuasiStatic.FluxTubes.Shapes.FixedShape.Cuboid
- Modelica.Magnetic.QuasiStatic.FluxTubes.Shapes.FixedShape.GenericFluxTube
- Modelica.Magnetic.QuasiStatic.FluxTubes.Shapes.FixedShape.HollowCylinderAxialFlux
- Modelica.Magnetic.QuasiStatic.FluxTubes.Shapes.FixedShape.HollowCylinderCircumferentialFlux
- Modelica.Magnetic.QuasiStatic.FluxTubes.Shapes.FixedShape.HollowCylinderRadialFlux
- Modelica.Magnetic.QuasiStatic.FluxTubes.Shapes.FixedShape.Toroid

### Magnetic.QuasiStatic.FluxTubes.Shapes.Leakage (9 个)

- Modelica.Magnetic.QuasiStatic.FluxTubes.Shapes.Leakage.CoaxCylindersEndFaces
- Modelica.Magnetic.QuasiStatic.FluxTubes.Shapes.Leakage.EighthOfHollowSphere
- Modelica.Magnetic.QuasiStatic.FluxTubes.Shapes.Leakage.EighthOfSphere
- Modelica.Magnetic.QuasiStatic.FluxTubes.Shapes.Leakage.HalfCylinder
- Modelica.Magnetic.QuasiStatic.FluxTubes.Shapes.Leakage.HalfHollowCylinder
- Modelica.Magnetic.QuasiStatic.FluxTubes.Shapes.Leakage.QuarterCylinder
- Modelica.Magnetic.QuasiStatic.FluxTubes.Shapes.Leakage.QuarterHollowCylinder
- Modelica.Magnetic.QuasiStatic.FluxTubes.Shapes.Leakage.QuarterHollowSphere
- Modelica.Magnetic.QuasiStatic.FluxTubes.Shapes.Leakage.QuarterSphere

### Magnetic.QuasiStatic.FluxTubes.Sources (4 个)

- Modelica.Magnetic.QuasiStatic.FluxTubes.Sources.ConstantMagneticFlux
- Modelica.Magnetic.QuasiStatic.FluxTubes.Sources.ConstantMagneticPotentialDifference
- Modelica.Magnetic.QuasiStatic.FluxTubes.Sources.SignalMagneticFlux
- Modelica.Magnetic.QuasiStatic.FluxTubes.Sources.SignalMagneticPotentialDifference

### Magnetic.QuasiStatic.FundamentalWave.BaseClasses (1 个)

- Modelica.Magnetic.QuasiStatic.FundamentalWave.BaseClasses.Machine

### Magnetic.QuasiStatic.FundamentalWave.BasicMachines.Components (6 个)

- Modelica.Magnetic.QuasiStatic.FundamentalWave.BasicMachines.Components.PermanentMagnet
- Modelica.Magnetic.QuasiStatic.FundamentalWave.BasicMachines.Components.QuasiStaticAnalogWinding
- Modelica.Magnetic.QuasiStatic.FundamentalWave.BasicMachines.Components.RotorSaliencyAirGap
- Modelica.Magnetic.QuasiStatic.FundamentalWave.BasicMachines.Components.SaliencyCageWinding
- Modelica.Magnetic.QuasiStatic.FundamentalWave.BasicMachines.Components.SymmetricPolyphaseCageWinding
- Modelica.Magnetic.QuasiStatic.FundamentalWave.BasicMachines.Components.SymmetricPolyphaseWinding

### Magnetic.QuasiStatic.FundamentalWave.BasicMachines.InductionMachines (2 个)

- Modelica.Magnetic.QuasiStatic.FundamentalWave.BasicMachines.InductionMachines.IM_SlipRing
- Modelica.Magnetic.QuasiStatic.FundamentalWave.BasicMachines.InductionMachines.IM_SquirrelCage

### Magnetic.QuasiStatic.FundamentalWave.BasicMachines.SynchronousMachines (3 个)

- Modelica.Magnetic.QuasiStatic.FundamentalWave.BasicMachines.SynchronousMachines.SM_ElectricalExcited
- Modelica.Magnetic.QuasiStatic.FundamentalWave.BasicMachines.SynchronousMachines.SM_PermanentMagnet
- Modelica.Magnetic.QuasiStatic.FundamentalWave.BasicMachines.SynchronousMachines.SM_ReluctanceRotor

### Magnetic.QuasiStatic.FundamentalWave.Components (9 个)

- Modelica.Magnetic.QuasiStatic.FundamentalWave.Components.Crossing
- Modelica.Magnetic.QuasiStatic.FundamentalWave.Components.EddyCurrent
- Modelica.Magnetic.QuasiStatic.FundamentalWave.Components.Ground
- Modelica.Magnetic.QuasiStatic.FundamentalWave.Components.Idle
- Modelica.Magnetic.QuasiStatic.FundamentalWave.Components.Permeance
- Modelica.Magnetic.QuasiStatic.FundamentalWave.Components.PolyphaseElectroMagneticConverter
- Modelica.Magnetic.QuasiStatic.FundamentalWave.Components.QuasiStaticAnalogElectroMagneticConverter
- Modelica.Magnetic.QuasiStatic.FundamentalWave.Components.Reluctance
- Modelica.Magnetic.QuasiStatic.FundamentalWave.Components.Short

### Magnetic.QuasiStatic.FundamentalWave.Examples.BasicMachines.InductionMachines (9 个)

- Modelica.Magnetic.QuasiStatic.FundamentalWave.Examples.BasicMachines.InductionMachines.IMC_Characteristics
- Modelica.Magnetic.QuasiStatic.FundamentalWave.Examples.BasicMachines.InductionMachines.IMC_Conveyor
- Modelica.Magnetic.QuasiStatic.FundamentalWave.Examples.BasicMachines.InductionMachines.IMC_DOL
- Modelica.Magnetic.QuasiStatic.FundamentalWave.Examples.BasicMachines.InductionMachines.IMC_Initialize
- Modelica.Magnetic.QuasiStatic.FundamentalWave.Examples.BasicMachines.InductionMachines.IMC_Transformer
- Modelica.Magnetic.QuasiStatic.FundamentalWave.Examples.BasicMachines.InductionMachines.IMC_withLosses
- Modelica.Magnetic.QuasiStatic.FundamentalWave.Examples.BasicMachines.InductionMachines.IMC_YD
- Modelica.Magnetic.QuasiStatic.FundamentalWave.Examples.BasicMachines.InductionMachines.IMS_Characteristics
- Modelica.Magnetic.QuasiStatic.FundamentalWave.Examples.BasicMachines.InductionMachines.IMS_Start

### Magnetic.QuasiStatic.FundamentalWave.Examples.BasicMachines.SynchronousMachines (6 个)

- Modelica.Magnetic.QuasiStatic.FundamentalWave.Examples.BasicMachines.SynchronousMachines.SMEE_Generator
- Modelica.Magnetic.QuasiStatic.FundamentalWave.Examples.BasicMachines.SynchronousMachines.SMPM_CurrentSource
- Modelica.Magnetic.QuasiStatic.FundamentalWave.Examples.BasicMachines.SynchronousMachines.SMPM_Mains
- Modelica.Magnetic.QuasiStatic.FundamentalWave.Examples.BasicMachines.SynchronousMachines.SMPM_MTPA
- Modelica.Magnetic.QuasiStatic.FundamentalWave.Examples.BasicMachines.SynchronousMachines.SMPM_OpenCircuit
- Modelica.Magnetic.QuasiStatic.FundamentalWave.Examples.BasicMachines.SynchronousMachines.SMR_CurrentSource

### Magnetic.QuasiStatic.FundamentalWave.Examples.Components (2 个)

- Modelica.Magnetic.QuasiStatic.FundamentalWave.Examples.Components.EddyCurrentLosses
- Modelica.Magnetic.QuasiStatic.FundamentalWave.Examples.Components.PolyphaseInductance

### Magnetic.QuasiStatic.FundamentalWave.Interfaces (2 个)

- Modelica.Magnetic.QuasiStatic.FundamentalWave.Interfaces.TwoPort
- Modelica.Magnetic.QuasiStatic.FundamentalWave.Interfaces.TwoPortElementary

### Magnetic.QuasiStatic.FundamentalWave.Losses (2 个)

- Modelica.Magnetic.QuasiStatic.FundamentalWave.Losses.PermanentMagnetLosses
- Modelica.Magnetic.QuasiStatic.FundamentalWave.Losses.StrayLoad

### Magnetic.QuasiStatic.FundamentalWave.Sensors (4 个)

- Modelica.Magnetic.QuasiStatic.FundamentalWave.Sensors.MagneticFluxSensor
- Modelica.Magnetic.QuasiStatic.FundamentalWave.Sensors.MagneticPotentialDifferenceSensor
- Modelica.Magnetic.QuasiStatic.FundamentalWave.Sensors.MagneticPotentialSensor
- Modelica.Magnetic.QuasiStatic.FundamentalWave.Sensors.RotorDisplacementAngle

### Magnetic.QuasiStatic.FundamentalWave.Sources (4 个)

- Modelica.Magnetic.QuasiStatic.FundamentalWave.Sources.ConstantFlux
- Modelica.Magnetic.QuasiStatic.FundamentalWave.Sources.ConstantMagneticPotentialDifference
- Modelica.Magnetic.QuasiStatic.FundamentalWave.Sources.SignalFlux
- Modelica.Magnetic.QuasiStatic.FundamentalWave.Sources.SignalMagneticPotentialDifference

### Magnetic.QuasiStatic.FundamentalWave.Utilities (6 个)

- Modelica.Magnetic.QuasiStatic.FundamentalWave.Utilities.CurrentController
- Modelica.Magnetic.QuasiStatic.FundamentalWave.Utilities.MultiTerminalBox
- Modelica.Magnetic.QuasiStatic.FundamentalWave.Utilities.SwitchedRheostat
- Modelica.Magnetic.QuasiStatic.FundamentalWave.Utilities.SwitchYD
- Modelica.Magnetic.QuasiStatic.FundamentalWave.Utilities.TerminalBox
- Modelica.Magnetic.QuasiStatic.FundamentalWave.Utilities.VfController

### Math (19 个)

- Modelica.Examples
- Modelica.LAPACK
- Modelica.Math
- Modelica.Math.Examples
- Modelica.Math.Examples
- Modelica.Math.FastFourierTransform
- Modelica.Math.Generators
- Modelica.Math.Nonlinear
- Modelica.Math.QuadratureLobatto3
- Modelica.Math.Random
- Modelica.Math.RealFFT1
- Modelica.Math.RealFFT2
- Modelica.Math.Utilities
- Modelica.Math.UtilityFunctions
- Modelica.Math.Xorshift1024star
- Modelica.Math.Xorshift128plus
- Modelica.Math.Xorshift64star
- Modelica.Matrices
- Modelica.Utilities

### Mechanics.MultiBody (2 个)

- Modelica.Mechanics.MultiBody
- Modelica.Mechanics.World

### Mechanics.MultiBody.Examples.Constraints (4 个)

- Modelica.Mechanics.MultiBody.Examples.Constraints.PrismaticConstraint
- Modelica.Mechanics.MultiBody.Examples.Constraints.RevoluteConstraint
- Modelica.Mechanics.MultiBody.Examples.Constraints.SphericalConstraint
- Modelica.Mechanics.MultiBody.Examples.Constraints.UniversalConstraint

### Mechanics.MultiBody.Examples.Elementary (20 个)

- Modelica.Mechanics.MultiBody.Examples.Elementary.DoublePendulum
- Modelica.Mechanics.MultiBody.Examples.Elementary.DoublePendulumInitTip
- Modelica.Mechanics.MultiBody.Examples.Elementary.ForceAndTorque
- Modelica.Mechanics.MultiBody.Examples.Elementary.FreeBody
- Modelica.Mechanics.MultiBody.Examples.Elementary.HeatLosses
- Modelica.Mechanics.MultiBody.Examples.Elementary.InitSpringConstant
- Modelica.Mechanics.MultiBody.Examples.Elementary.LineForceWithTwoMasses
- Modelica.Mechanics.MultiBody.Examples.Elementary.Pendulum
- Modelica.Mechanics.MultiBody.Examples.Elementary.PendulumWithSpringDamper
- Modelica.Mechanics.MultiBody.Examples.Elementary.PointGravityWithPointMasses
- Modelica.Mechanics.MultiBody.Examples.Elementary.PointGravityWithPointMasses2
- Modelica.Mechanics.MultiBody.Examples.Elementary.RollingWheelSetDriving
- Modelica.Mechanics.MultiBody.Examples.Elementary.RollingWheelSetPulling
- Modelica.Mechanics.MultiBody.Examples.Elementary.SpringDamperSystem
- Modelica.Mechanics.MultiBody.Examples.Elementary.SpringMassSystem
- Modelica.Mechanics.MultiBody.Examples.Elementary.SpringWithMass
- Modelica.Mechanics.MultiBody.Examples.Elementary.Surfaces
- Modelica.Mechanics.MultiBody.Examples.Elementary.SystemWithStandardBodies
- Modelica.Mechanics.MultiBody.Examples.Elementary.ThreeSprings
- Modelica.Mechanics.MultiBody.Examples.Elementary.UserDefinedGravityField

### Mechanics.MultiBody.Examples.Loops (11 个)

- Modelica.Mechanics.MultiBody.Examples.Loops
- Modelica.Mechanics.MultiBody.Examples.Loops.Engine1a
- Modelica.Mechanics.MultiBody.Examples.Loops.Engine1b
- Modelica.Mechanics.MultiBody.Examples.Loops.Engine1b_analytic
- Modelica.Mechanics.MultiBody.Examples.Loops.EngineV6
- Modelica.Mechanics.MultiBody.Examples.Loops.EngineV6_analytic
- Modelica.Mechanics.MultiBody.Examples.Loops.Fourbar_analytic
- Modelica.Mechanics.MultiBody.Examples.Loops.Fourbar1
- Modelica.Mechanics.MultiBody.Examples.Loops.Fourbar2
- Modelica.Mechanics.MultiBody.Examples.Loops.PlanarFourbar
- Modelica.Mechanics.MultiBody.Examples.Loops.PlanarLoops_analytic

### Mechanics.MultiBody.Examples.Loops.Utilities (6 个)

- Modelica.Mechanics.MultiBody.Examples.Loops.Utilities.Cylinder
- Modelica.Mechanics.MultiBody.Examples.Loops.Utilities.Cylinder_analytic_CAD
- Modelica.Mechanics.MultiBody.Examples.Loops.Utilities.CylinderBase
- Modelica.Mechanics.MultiBody.Examples.Loops.Utilities.Engine1Base
- Modelica.Mechanics.MultiBody.Examples.Loops.Utilities.EngineV6_analytic
- Modelica.Mechanics.MultiBody.Examples.Loops.Utilities.GasForce2

### Mechanics.MultiBody.Examples.Rotational3DEffects (5 个)

- Modelica.Mechanics.MultiBody.Examples.Rotational3DEffects.ActuatedDrive
- Modelica.Mechanics.MultiBody.Examples.Rotational3DEffects.BevelGear1D
- Modelica.Mechanics.MultiBody.Examples.Rotational3DEffects.GearConstraint
- Modelica.Mechanics.MultiBody.Examples.Rotational3DEffects.GyroscopicEffects
- Modelica.Mechanics.MultiBody.Examples.Rotational3DEffects.MovingActuatedDrive

### Mechanics.MultiBody.Examples.Systems.RobotR3 (2 个)

- Modelica.Mechanics.MultiBody.Examples.Systems.RobotR3.FullRobot
- Modelica.Mechanics.MultiBody.Examples.Systems.RobotR3.OneAxis

### Mechanics.MultiBody.Examples.Systems.RobotR3.Utilities (9 个)

- Modelica.Mechanics.MultiBody.Examples.Systems.RobotR3.Utilities.AxisType2
- Modelica.Mechanics.MultiBody.Examples.Systems.RobotR3.Utilities.Controller
- Modelica.Mechanics.MultiBody.Examples.Systems.RobotR3.Utilities.GearType1
- Modelica.Mechanics.MultiBody.Examples.Systems.RobotR3.Utilities.GearType2
- Modelica.Mechanics.MultiBody.Examples.Systems.RobotR3.Utilities.MechanicalStructure
- Modelica.Mechanics.MultiBody.Examples.Systems.RobotR3.Utilities.Motor
- Modelica.Mechanics.MultiBody.Examples.Systems.RobotR3.Utilities.PathPlanning1
- Modelica.Mechanics.MultiBody.Examples.Systems.RobotR3.Utilities.PathPlanning6
- Modelica.Mechanics.MultiBody.Examples.Systems.RobotR3.Utilities.PathToAxisControlBus

### Mechanics.MultiBody.Forces (12 个)

- Modelica.Mechanics.MultiBody.Forces.Damper
- Modelica.Mechanics.MultiBody.Forces.Force
- Modelica.Mechanics.MultiBody.Forces.ForceAndTorque
- Modelica.Mechanics.MultiBody.Forces.LineForceWithMass
- Modelica.Mechanics.MultiBody.Forces.LineForceWithTwoMasses
- Modelica.Mechanics.MultiBody.Forces.Spring
- Modelica.Mechanics.MultiBody.Forces.SpringDamperParallel
- Modelica.Mechanics.MultiBody.Forces.SpringDamperSeries
- Modelica.Mechanics.MultiBody.Forces.Torque
- Modelica.Mechanics.MultiBody.Forces.WorldForce
- Modelica.Mechanics.MultiBody.Forces.WorldForceAndTorque
- Modelica.Mechanics.MultiBody.Forces.WorldTorque

### Mechanics.MultiBody.Forces.Internal (5 个)

- Modelica.Mechanics.MultiBody.Forces.Internal.BasicForce
- Modelica.Mechanics.MultiBody.Forces.Internal.BasicTorque
- Modelica.Mechanics.MultiBody.Forces.Internal.BasicWorldForce
- Modelica.Mechanics.MultiBody.Forces.Internal.BasicWorldTorque
- Modelica.Mechanics.MultiBody.Forces.Internal.ZeroForceAndTorque

### Mechanics.MultiBody.Interfaces (13 个)

- Modelica.Mechanics.MultiBody.Interfaces.FlangeWithBearingAdaptor
- Modelica.Mechanics.MultiBody.Interfaces.LineForceBase
- Modelica.Mechanics.MultiBody.Interfaces.PartialAbsoluteSensor
- Modelica.Mechanics.MultiBody.Interfaces.PartialElementaryJoint
- Modelica.Mechanics.MultiBody.Interfaces.PartialForce
- Modelica.Mechanics.MultiBody.Interfaces.PartialLineForce
- Modelica.Mechanics.MultiBody.Interfaces.PartialOneFrame_a
- Modelica.Mechanics.MultiBody.Interfaces.PartialOneFrame_b
- Modelica.Mechanics.MultiBody.Interfaces.PartialRelativeSensor
- Modelica.Mechanics.MultiBody.Interfaces.PartialTwoFrames
- Modelica.Mechanics.MultiBody.Interfaces.PartialTwoFramesDoubleSize
- Modelica.Mechanics.MultiBody.Interfaces.PartialVisualizer
- Modelica.Mechanics.MultiBody.Interfaces.ZeroPosition

### Mechanics.MultiBody.Joints (14 个)

- Modelica.Mechanics.MultiBody.Joints.Cylindrical
- Modelica.Mechanics.MultiBody.Joints.FreeMotion
- Modelica.Mechanics.MultiBody.Joints.FreeMotionScalarInit
- Modelica.Mechanics.MultiBody.Joints.GearConstraint
- Modelica.Mechanics.MultiBody.Joints.Planar
- Modelica.Mechanics.MultiBody.Joints.Prismatic
- Modelica.Mechanics.MultiBody.Joints.Revolute
- Modelica.Mechanics.MultiBody.Joints.RevolutePlanarLoopConstraint
- Modelica.Mechanics.MultiBody.Joints.RollingWheel
- Modelica.Mechanics.MultiBody.Joints.RollingWheelSet
- Modelica.Mechanics.MultiBody.Joints.Spherical
- Modelica.Mechanics.MultiBody.Joints.SphericalSpherical
- Modelica.Mechanics.MultiBody.Joints.Universal
- Modelica.Mechanics.MultiBody.Joints.UniversalSpherical

### Mechanics.MultiBody.Joints.Assemblies (7 个)

- Modelica.Mechanics.MultiBody.Joints.Assemblies.JointRRP
- Modelica.Mechanics.MultiBody.Joints.Assemblies.JointRRR
- Modelica.Mechanics.MultiBody.Joints.Assemblies.JointSSP
- Modelica.Mechanics.MultiBody.Joints.Assemblies.JointSSR
- Modelica.Mechanics.MultiBody.Joints.Assemblies.JointUPS
- Modelica.Mechanics.MultiBody.Joints.Assemblies.JointUSP
- Modelica.Mechanics.MultiBody.Joints.Assemblies.JointUSR

### Mechanics.MultiBody.Joints.Constraints (4 个)

- Modelica.Mechanics.MultiBody.Joints.Constraints.Prismatic
- Modelica.Mechanics.MultiBody.Joints.Constraints.Revolute
- Modelica.Mechanics.MultiBody.Joints.Constraints.Spherical
- Modelica.Mechanics.MultiBody.Joints.Constraints.Universal

### Mechanics.MultiBody.Joints.Internal (6 个)

- Modelica.Mechanics.MultiBody.Joints.Internal.InitAngle
- Modelica.Mechanics.MultiBody.Joints.Internal.InitAngularVelocity
- Modelica.Mechanics.MultiBody.Joints.Internal.InitPosition
- Modelica.Mechanics.MultiBody.Joints.Internal.PrismaticWithLengthConstraint
- Modelica.Mechanics.MultiBody.Joints.Internal.RevoluteWithLengthConstraint
- Modelica.Mechanics.MultiBody.Joints.Internal.RollingConstraintVerticalWheel

### Mechanics.MultiBody.Parts (16 个)

- Modelica.Mechanics.MultiBody.Parts.BevelGear1D
- Modelica.Mechanics.MultiBody.Parts.Body
- Modelica.Mechanics.MultiBody.Parts.BodyBox
- Modelica.Mechanics.MultiBody.Parts.BodyCylinder
- Modelica.Mechanics.MultiBody.Parts.BodyShape
- Modelica.Mechanics.MultiBody.Parts.Fixed
- Modelica.Mechanics.MultiBody.Parts.FixedRotation
- Modelica.Mechanics.MultiBody.Parts.FixedTranslation
- Modelica.Mechanics.MultiBody.Parts.Housing
- Modelica.Mechanics.MultiBody.Parts.Housing
- Modelica.Mechanics.MultiBody.Parts.Mounting1D
- Modelica.Mechanics.MultiBody.Parts.PointMass
- Modelica.Mechanics.MultiBody.Parts.RollingWheel
- Modelica.Mechanics.MultiBody.Parts.RollingWheelSet
- Modelica.Mechanics.MultiBody.Parts.Rotor1D
- Modelica.Mechanics.MultiBody.Parts.RotorWith3DEffects

### Mechanics.MultiBody.Sensors (17 个)

- Modelica.Mechanics.MultiBody.Sensors.AbsoluteAngles
- Modelica.Mechanics.MultiBody.Sensors.AbsoluteAngularVelocity
- Modelica.Mechanics.MultiBody.Sensors.AbsolutePosition
- Modelica.Mechanics.MultiBody.Sensors.AbsoluteSensor
- Modelica.Mechanics.MultiBody.Sensors.AbsoluteVelocity
- Modelica.Mechanics.MultiBody.Sensors.CutForce
- Modelica.Mechanics.MultiBody.Sensors.CutForceAndTorque
- Modelica.Mechanics.MultiBody.Sensors.CutTorque
- Modelica.Mechanics.MultiBody.Sensors.Distance
- Modelica.Mechanics.MultiBody.Sensors.Power
- Modelica.Mechanics.MultiBody.Sensors.RelativeAngles
- Modelica.Mechanics.MultiBody.Sensors.RelativeAngularVelocity
- Modelica.Mechanics.MultiBody.Sensors.RelativePosition
- Modelica.Mechanics.MultiBody.Sensors.RelativeSensor
- Modelica.Mechanics.MultiBody.Sensors.RelativeVelocity
- Modelica.Mechanics.MultiBody.Sensors.TransformAbsoluteVector
- Modelica.Mechanics.MultiBody.Sensors.TransformRelativeVector

### Mechanics.MultiBody.Sensors.Internal (14 个)

- Modelica.Mechanics.MultiBody.Sensors.Internal.BasicAbsoluteAngularVelocity
- Modelica.Mechanics.MultiBody.Sensors.Internal.BasicAbsolutePosition
- Modelica.Mechanics.MultiBody.Sensors.Internal.BasicCutForce
- Modelica.Mechanics.MultiBody.Sensors.Internal.BasicCutTorque
- Modelica.Mechanics.MultiBody.Sensors.Internal.BasicRelativeAngularVelocity
- Modelica.Mechanics.MultiBody.Sensors.Internal.BasicRelativePosition
- Modelica.Mechanics.MultiBody.Sensors.Internal.BasicTransformAbsoluteVector
- Modelica.Mechanics.MultiBody.Sensors.Internal.BasicTransformRelativeVector
- Modelica.Mechanics.MultiBody.Sensors.Internal.PartialAbsoluteBaseSensor
- Modelica.Mechanics.MultiBody.Sensors.Internal.PartialAbsoluteSensor
- Modelica.Mechanics.MultiBody.Sensors.Internal.PartialCutForceBaseSensor
- Modelica.Mechanics.MultiBody.Sensors.Internal.PartialCutForceSensor
- Modelica.Mechanics.MultiBody.Sensors.Internal.PartialRelativeBaseSensor
- Modelica.Mechanics.MultiBody.Sensors.Internal.PartialRelativeSensor

### Mechanics.MultiBody.UsersGuide.Tutorial (2 个)

- Modelica.Mechanics.MultiBody.UsersGuide.Tutorial.ConnectionOfLineForces
- Modelica.Mechanics.MultiBody.UsersGuide.Tutorial.OverView

### Mechanics.MultiBody.UsersGuide.Tutorial.LoopStructures (3 个)

- Modelica.Mechanics.MultiBody.UsersGuide.Tutorial.LoopStructures
- Modelica.Mechanics.MultiBody.UsersGuide.Tutorial.LoopStructures.AnalyticLoopHandling
- Modelica.Mechanics.MultiBody.UsersGuide.Tutorial.LoopStructures.Introduction

### Mechanics.MultiBody.Visualizers (9 个)

- Modelica.Mechanics.MultiBody.Visualizers.FixedArrow
- Modelica.Mechanics.MultiBody.Visualizers.FixedFrame
- Modelica.Mechanics.MultiBody.Visualizers.FixedShape
- Modelica.Mechanics.MultiBody.Visualizers.FixedShape2
- Modelica.Mechanics.MultiBody.Visualizers.PipeWithScalarField
- Modelica.Mechanics.MultiBody.Visualizers.Rectangle
- Modelica.Mechanics.MultiBody.Visualizers.SignalArrow
- Modelica.Mechanics.MultiBody.Visualizers.Torus
- Modelica.Mechanics.MultiBody.Visualizers.VoluminousWheel

### Mechanics.MultiBody.Visualizers.Advanced (4 个)

- Modelica.Mechanics.MultiBody.Visualizers.Advanced.Arrow
- Modelica.Mechanics.MultiBody.Visualizers.Advanced.DoubleArrow
- Modelica.Mechanics.MultiBody.Visualizers.Advanced.Shape
- Modelica.Mechanics.MultiBody.Visualizers.Advanced.Vector

### Mechanics.MultiBody.Visualizers.Internal (1 个)

- Modelica.Mechanics.MultiBody.Visualizers.Internal.FixedLines

### Mechanics.Rotational.Components (30 个)

- Modelica.Mechanics.Rotational.Components.AngleToTorqueAdaptor
- Modelica.Mechanics.Rotational.Components.BearingFriction
- Modelica.Mechanics.Rotational.Components.Brake
- Modelica.Mechanics.Rotational.Components.Clutch
- Modelica.Mechanics.Rotational.Components.Damper
- Modelica.Mechanics.Rotational.Components.Disc
- Modelica.Mechanics.Rotational.Components.ElastoBacklash
- Modelica.Mechanics.Rotational.Components.ElastoBacklash2
- Modelica.Mechanics.Rotational.Components.Fixed
- Modelica.Mechanics.Rotational.Components.Gearbox
- Modelica.Mechanics.Rotational.Components.GeneralAngleToTorqueAdaptor
- Modelica.Mechanics.Rotational.Components.GeneralTorqueToAngleAdaptor
- Modelica.Mechanics.Rotational.Components.IdealGear
- Modelica.Mechanics.Rotational.Components.IdealGearR2T
- Modelica.Mechanics.Rotational.Components.IdealPlanetary
- Modelica.Mechanics.Rotational.Components.IdealRollingWheel
- Modelica.Mechanics.Rotational.Components.Inertia
- Modelica.Mechanics.Rotational.Components.InitializeFlange
- Modelica.Mechanics.Rotational.Components.LossyGear
- Modelica.Mechanics.Rotational.Components.Move_phi
- Modelica.Mechanics.Rotational.Components.Move_w
- Modelica.Mechanics.Rotational.Components.OneWayClutch
- Modelica.Mechanics.Rotational.Components.RelativeStates
- Modelica.Mechanics.Rotational.Components.Set_a_start
- Modelica.Mechanics.Rotational.Components.Set_flange_tau
- Modelica.Mechanics.Rotational.Components.Set_phi_start
- Modelica.Mechanics.Rotational.Components.Set_w_start
- Modelica.Mechanics.Rotational.Components.Spring
- Modelica.Mechanics.Rotational.Components.SpringDamper
- Modelica.Mechanics.Rotational.Components.TorqueToAngleAdaptor

### Mechanics.Rotational.Examples (16 个)

- Modelica.Mechanics.Rotational.Examples.Backlash
- Modelica.Mechanics.Rotational.Examples.CoupledClutches
- Modelica.Mechanics.Rotational.Examples.EddyCurrentBrake
- Modelica.Mechanics.Rotational.Examples.ElasticBearing
- Modelica.Mechanics.Rotational.Examples.First
- Modelica.Mechanics.Rotational.Examples.FirstGrounded
- Modelica.Mechanics.Rotational.Examples.Friction
- Modelica.Mechanics.Rotational.Examples.GenerationOfFMUs
- Modelica.Mechanics.Rotational.Examples.HeatLosses
- Modelica.Mechanics.Rotational.Examples.LossyGearDemo1
- Modelica.Mechanics.Rotational.Examples.LossyGearDemo2
- Modelica.Mechanics.Rotational.Examples.LossyGearDemo3
- Modelica.Mechanics.Rotational.Examples.OneWayClutch
- Modelica.Mechanics.Rotational.Examples.OneWayClutchDisengaged
- Modelica.Mechanics.Rotational.Examples.RollingWheel
- Modelica.Mechanics.Rotational.Examples.SimpleGearShift

### Mechanics.Rotational.Examples.Utilities (5 个)

- Modelica.Mechanics.Rotational.Examples.Utilities.DirectInertia
- Modelica.Mechanics.Rotational.Examples.Utilities.InverseInertia
- Modelica.Mechanics.Rotational.Examples.Utilities.Spring
- Modelica.Mechanics.Rotational.Examples.Utilities.SpringDamper
- Modelica.Mechanics.Rotational.Examples.Utilities.SpringDamperNoRelativeStates

### Mechanics.Rotational.Interfaces (12 个)

- Modelica.Mechanics.Rotational.Interfaces.InternalSupport
- Modelica.Mechanics.Rotational.Interfaces.PartialAbsoluteSensor
- Modelica.Mechanics.Rotational.Interfaces.PartialCompliant
- Modelica.Mechanics.Rotational.Interfaces.PartialCompliantWithRelativeStates
- Modelica.Mechanics.Rotational.Interfaces.PartialElementaryOneFlangeAndSupport2
- Modelica.Mechanics.Rotational.Interfaces.PartialElementaryRotationalToTranslational
- Modelica.Mechanics.Rotational.Interfaces.PartialElementaryTwoFlangesAndSupport2
- Modelica.Mechanics.Rotational.Interfaces.PartialFriction
- Modelica.Mechanics.Rotational.Interfaces.PartialOneFlangeAndSupport
- Modelica.Mechanics.Rotational.Interfaces.PartialRelativeSensor
- Modelica.Mechanics.Rotational.Interfaces.PartialTorque
- Modelica.Mechanics.Rotational.Interfaces.PartialTwoFlangesAndSupport

### Mechanics.Rotational.Sensors (9 个)

- Modelica.Mechanics.Rotational.Sensors.AccSensor
- Modelica.Mechanics.Rotational.Sensors.AngleSensor
- Modelica.Mechanics.Rotational.Sensors.MultiSensor
- Modelica.Mechanics.Rotational.Sensors.PowerSensor
- Modelica.Mechanics.Rotational.Sensors.RelAccSensor
- Modelica.Mechanics.Rotational.Sensors.RelAngleSensor
- Modelica.Mechanics.Rotational.Sensors.RelSpeedSensor
- Modelica.Mechanics.Rotational.Sensors.SpeedSensor
- Modelica.Mechanics.Rotational.Sensors.TorqueSensor

### Mechanics.Rotational.Sources (13 个)

- Modelica.Mechanics.Rotational.Sources.Accelerate
- Modelica.Mechanics.Rotational.Sources.ConstantSpeed
- Modelica.Mechanics.Rotational.Sources.ConstantTorque
- Modelica.Mechanics.Rotational.Sources.EddyCurrentTorque
- Modelica.Mechanics.Rotational.Sources.LinearSpeedDependentTorque
- Modelica.Mechanics.Rotational.Sources.Move
- Modelica.Mechanics.Rotational.Sources.Position
- Modelica.Mechanics.Rotational.Sources.QuadraticSpeedDependentTorque
- Modelica.Mechanics.Rotational.Sources.SignTorque
- Modelica.Mechanics.Rotational.Sources.Speed
- Modelica.Mechanics.Rotational.Sources.Torque
- Modelica.Mechanics.Rotational.Sources.Torque2
- Modelica.Mechanics.Rotational.Sources.TorqueStep

### Mechanics.Rotational.UsersGuide (1 个)

- Modelica.Mechanics.Rotational.UsersGuide.ModelingOfFriction

### Mechanics.Translational.Components (20 个)

- Modelica.Mechanics.Translational.Components.Brake
- Modelica.Mechanics.Translational.Components.Damper
- Modelica.Mechanics.Translational.Components.ElastoGap
- Modelica.Mechanics.Translational.Components.Fixed
- Modelica.Mechanics.Translational.Components.GeneralForceToPositionAdaptor
- Modelica.Mechanics.Translational.Components.GeneralPositionToForceAdaptor
- Modelica.Mechanics.Translational.Components.InitializeFlange
- Modelica.Mechanics.Translational.Components.Mass
- Modelica.Mechanics.Translational.Components.MassWithStopAndFriction
- Modelica.Mechanics.Translational.Components.RelativeStates
- Modelica.Mechanics.Translational.Components.Rod
- Modelica.Mechanics.Translational.Components.RollingResistance
- Modelica.Mechanics.Translational.Components.Set_a_start
- Modelica.Mechanics.Translational.Components.Set_flange_f
- Modelica.Mechanics.Translational.Components.Set_s_start
- Modelica.Mechanics.Translational.Components.Set_v_start
- Modelica.Mechanics.Translational.Components.Spring
- Modelica.Mechanics.Translational.Components.SpringDamper
- Modelica.Mechanics.Translational.Components.SupportFriction
- Modelica.Mechanics.Translational.Components.Vehicle

### Mechanics.Translational.Examples (15 个)

- Modelica.Mechanics.Translational.Examples.Accelerate
- Modelica.Mechanics.Translational.Examples.Brake
- Modelica.Mechanics.Translational.Examples.Damper
- Modelica.Mechanics.Translational.Examples.EddyCurrentBrake
- Modelica.Mechanics.Translational.Examples.ElastoGap
- Modelica.Mechanics.Translational.Examples.Friction
- Modelica.Mechanics.Translational.Examples.GenerationOfFMUs
- Modelica.Mechanics.Translational.Examples.HeatLosses
- Modelica.Mechanics.Translational.Examples.InitialConditions
- Modelica.Mechanics.Translational.Examples.Oscillator
- Modelica.Mechanics.Translational.Examples.PreLoad
- Modelica.Mechanics.Translational.Examples.Sensors
- Modelica.Mechanics.Translational.Examples.SignConvention
- Modelica.Mechanics.Translational.Examples.Vehicle
- Modelica.Mechanics.Translational.Examples.WhyArrows

### Mechanics.Translational.Examples.Utilities (5 个)

- Modelica.Mechanics.Translational.Examples.Utilities.DirectMass
- Modelica.Mechanics.Translational.Examples.Utilities.InverseMass
- Modelica.Mechanics.Translational.Examples.Utilities.Spring
- Modelica.Mechanics.Translational.Examples.Utilities.SpringDamper
- Modelica.Mechanics.Translational.Examples.Utilities.SpringDamperNoRelativeStates

### Mechanics.Translational.Interfaces (12 个)

- Modelica.Mechanics.Translational.Interfaces.InternalSupport
- Modelica.Mechanics.Translational.Interfaces.PartialAbsoluteSensor
- Modelica.Mechanics.Translational.Interfaces.PartialCompliant
- Modelica.Mechanics.Translational.Interfaces.PartialCompliantWithRelativeStates
- Modelica.Mechanics.Translational.Interfaces.PartialElementaryOneFlangeAndSupport2
- Modelica.Mechanics.Translational.Interfaces.PartialElementaryTwoFlangesAndSupport2
- Modelica.Mechanics.Translational.Interfaces.PartialForce
- Modelica.Mechanics.Translational.Interfaces.PartialFriction
- Modelica.Mechanics.Translational.Interfaces.PartialOneFlangeAndSupport
- Modelica.Mechanics.Translational.Interfaces.PartialRelativeSensor
- Modelica.Mechanics.Translational.Interfaces.PartialRigid
- Modelica.Mechanics.Translational.Interfaces.PartialTwoFlangesAndSupport

### Mechanics.Translational.Sensors (9 个)

- Modelica.Mechanics.Translational.Sensors.AccSensor
- Modelica.Mechanics.Translational.Sensors.ForceSensor
- Modelica.Mechanics.Translational.Sensors.MultiSensor
- Modelica.Mechanics.Translational.Sensors.PositionSensor
- Modelica.Mechanics.Translational.Sensors.PowerSensor
- Modelica.Mechanics.Translational.Sensors.RelAccSensor
- Modelica.Mechanics.Translational.Sensors.RelPositionSensor
- Modelica.Mechanics.Translational.Sensors.RelSpeedSensor
- Modelica.Mechanics.Translational.Sensors.SpeedSensor

### Mechanics.Translational.Sources (13 个)

- Modelica.Mechanics.Translational.Sources.Accelerate
- Modelica.Mechanics.Translational.Sources.ConstantForce
- Modelica.Mechanics.Translational.Sources.ConstantSpeed
- Modelica.Mechanics.Translational.Sources.EddyCurrentForce
- Modelica.Mechanics.Translational.Sources.Force
- Modelica.Mechanics.Translational.Sources.Force2
- Modelica.Mechanics.Translational.Sources.ForceStep
- Modelica.Mechanics.Translational.Sources.LinearSpeedDependentForce
- Modelica.Mechanics.Translational.Sources.Move
- Modelica.Mechanics.Translational.Sources.Position
- Modelica.Mechanics.Translational.Sources.QuadraticSpeedDependentForce
- Modelica.Mechanics.Translational.Sources.SignForce
- Modelica.Mechanics.Translational.Sources.Speed

### Media (65 个)

- Modelica.BalanceVolume
- Modelica.BasicStructure
- Modelica.BasicUsage
- Modelica.BridgmansTables
- Modelica.BridgmansTablesForWater
- Modelica.Common
- Modelica.Constants
- Modelica.Examples
- Modelica.ExtendedProperties
- Modelica.FixedAmbient
- Modelica.FixedMassFlowRate
- Modelica.FluidConstants
- Modelica.FluidConstants
- Modelica.IdealGas
- Modelica.IdealGasH2O
- Modelica.Initialization
- Modelica.Interfaces
- Modelica.Inverse_sh_T
- Modelica.Inverse_sh_T
- Modelica.Inverse_sh_TX
- Modelica.Inverse_sh_TX
- Modelica.Inverse_sine
- Modelica.InverseIncompressible_sh_T
- Modelica.Media
- Modelica.Media.Common
- Modelica.Media.EOSIdealCoeff
- Modelica.Media.EOSResidualCoeff
- Modelica.Media.Examples
- Modelica.Media.Incompressible
- Modelica.Media.R134a
- Modelica.Media.R134a_ph
- Modelica.Media.T_props_ph
- Modelica.Media.TableBased
- Modelica.Media.TestGlycol
- Modelica.MediumDefinition
- Modelica.MediumUsage
- Modelica.MixtureGases
- Modelica.MoistAir
- Modelica.MoistAir
- Modelica.OptionalProperties
- Modelica.PartialLinearFluid
- Modelica.PartialMedium
- Modelica.PartialMedium
- Modelica.PartialSimpleIdealGasMedium
- Modelica.PartialSimpleMedium
- Modelica.PartialTestModel
- Modelica.PartialTestModel2
- Modelica.PortVolume
- Modelica.PsychrometricData
- Modelica.ReferenceAir
- Modelica.ShortPipe
- Modelica.ShortPipe
- Modelica.SimpleLiquidWater
- Modelica.SolveOneNonlinearEquation
- Modelica.SpecificEnthalpyAsFunction
- Modelica.StaticStateSelection
- Modelica.TemplateMedium
- Modelica.TestTwoPhaseStates
- Modelica.TwoPhase
- Modelica.TwoPhase
- Modelica.TwoPhaseWater
- Modelica.Types
- Modelica.UsersGuide
- Modelica.Utilities
- Modelica.WaterIF97

### Media.Air (11 个)

- Modelica.Media.Air.Air_Base
- Modelica.Media.Air.Air_Utilities
- Modelica.Media.Air.Basic
- Modelica.Media.Air.Basic
- Modelica.Media.Air.Gibbs
- Modelica.Media.Air.Helmholtz
- Modelica.Media.Air.Ice09_Utilities
- Modelica.Media.Air.MoistAir
- Modelica.Media.Air.ReferenceAir
- Modelica.Media.Air.ReferenceMoistAir
- Modelica.Media.Air.Utilities

### Media.IdealGases.Common (4 个)

- Modelica.Media.IdealGases.Common
- Modelica.Media.IdealGases.Common.Functions
- Modelica.Media.IdealGases.MixtureGasNasa
- Modelica.Media.IdealGases.SingleGasNasa

### Media.Water (5 个)

- Modelica.Media.Water
- Modelica.Media.Water.BaseIF97
- Modelica.Media.Water.IF97_Utilities
- Modelica.Media.Water.Transport
- Modelica.Media.WaterIF97_base

### Resources.Images.Media.UsersGuide (1 个)

- Modelica.Resources.Images.Media.UsersGuide.Demo

### Root (55 个)

- Modelica.Alternative
- Modelica.BooleanStep
- Modelica.Cases
- Modelica.ComparisonWithStateGraph2
- Modelica.CompositeStep
- Modelica.CompositeStep1
- Modelica.CompositeStep2
- Modelica.CompositeStepState
- Modelica.Connectors
- Modelica.ControlledTanks
- Modelica.Conventions
- Modelica.Documentation
- Modelica.Equations
- Modelica.Examples
- Modelica.ExecutionPaths
- Modelica.FirstExample
- Modelica.FirstExample_Variant2
- Modelica.FirstExample_Variant3
- Modelica.Format
- Modelica.HowToUseUnits
- Modelica.InitialStep
- Modelica.InitialStepWithSignal
- Modelica.Interfaces
- Modelica.MakeProduct
- Modelica.Modelica
- Modelica.ModelicaCode
- Modelica.OuterState
- Modelica.OuterStatePort
- Modelica.OverView
- Modelica.Parallel
- Modelica.ParameterDefaults
- Modelica.PartialCompositeStep
- Modelica.PartialStep
- Modelica.PartialTransition
- Modelica.ReleaseNotes
- Modelica.ShowCompositeStep
- Modelica.ShowExceptions
- Modelica.Source
- Modelica.StateGraph
- Modelica.StateGraphRoot
- Modelica.Step
- Modelica.StepWithSignal
- Modelica.Tank
- Modelica.TankController
- Modelica.Units
- Modelica.UsersGuide
- Modelica.UsersGuide
- Modelica.UsersGuide
- Modelica.Utilities
- Modelica.Version_1_5
- Modelica.Version_2_2_1
- Modelica.Version_2_2_2
- Modelica.Version_3_0
- Modelica.Version_3_2
- Modelica.VersionManagement

### Thermal.FluidHeatFlow.BaseClasses (4 个)

- Modelica.Thermal.FluidHeatFlow.BaseClasses.SimpleFriction
- Modelica.Thermal.FluidHeatFlow.BaseClasses.SinglePortBottom
- Modelica.Thermal.FluidHeatFlow.BaseClasses.SinglePortLeft
- Modelica.Thermal.FluidHeatFlow.BaseClasses.TwoPort

### Thermal.FluidHeatFlow.Components (5 个)

- Modelica.Thermal.FluidHeatFlow.Components.Cylinder
- Modelica.Thermal.FluidHeatFlow.Components.OneWayValve
- Modelica.Thermal.FluidHeatFlow.Components.OpenTank
- Modelica.Thermal.FluidHeatFlow.Components.Pipe
- Modelica.Thermal.FluidHeatFlow.Components.Valve

### Thermal.FluidHeatFlow.Examples (12 个)

- Modelica.Thermal.FluidHeatFlow.Examples.IndirectCooling
- Modelica.Thermal.FluidHeatFlow.Examples.OneMass
- Modelica.Thermal.FluidHeatFlow.Examples.ParallelCooling
- Modelica.Thermal.FluidHeatFlow.Examples.ParallelPumpDropOut
- Modelica.Thermal.FluidHeatFlow.Examples.PumpAndValve
- Modelica.Thermal.FluidHeatFlow.Examples.PumpDropOut
- Modelica.Thermal.FluidHeatFlow.Examples.SimpleCooling
- Modelica.Thermal.FluidHeatFlow.Examples.TestCylinder
- Modelica.Thermal.FluidHeatFlow.Examples.TestOpenTank
- Modelica.Thermal.FluidHeatFlow.Examples.TwoMass
- Modelica.Thermal.FluidHeatFlow.Examples.TwoTanks
- Modelica.Thermal.FluidHeatFlow.Examples.WaterPump

### Thermal.FluidHeatFlow.Examples.Utilities (1 个)

- Modelica.Thermal.FluidHeatFlow.Examples.Utilities.DoubleRamp

### Thermal.FluidHeatFlow.Interfaces (3 个)

- Modelica.Thermal.FluidHeatFlow.Interfaces.AbsoluteSensor
- Modelica.Thermal.FluidHeatFlow.Interfaces.FlowSensor
- Modelica.Thermal.FluidHeatFlow.Interfaces.RelativeSensorBase

### Thermal.FluidHeatFlow.Sensors (7 个)

- Modelica.Thermal.FluidHeatFlow.Sensors.EnthalpyFlowSensor
- Modelica.Thermal.FluidHeatFlow.Sensors.MassFlowSensor
- Modelica.Thermal.FluidHeatFlow.Sensors.PressureSensor
- Modelica.Thermal.FluidHeatFlow.Sensors.RelPressureSensor
- Modelica.Thermal.FluidHeatFlow.Sensors.RelTemperatureSensor
- Modelica.Thermal.FluidHeatFlow.Sensors.TemperatureSensor
- Modelica.Thermal.FluidHeatFlow.Sensors.VolumeFlowSensor

### Thermal.FluidHeatFlow.Sources (5 个)

- Modelica.Thermal.FluidHeatFlow.Sources.AbsolutePressure
- Modelica.Thermal.FluidHeatFlow.Sources.Ambient
- Modelica.Thermal.FluidHeatFlow.Sources.IdealPump
- Modelica.Thermal.FluidHeatFlow.Sources.PressureIncrease
- Modelica.Thermal.FluidHeatFlow.Sources.VolumeFlow

### Thermal.HeatTransfer.Celsius (5 个)

- Modelica.Thermal.HeatTransfer.Celsius.FixedTemperature
- Modelica.Thermal.HeatTransfer.Celsius.FromKelvin
- Modelica.Thermal.HeatTransfer.Celsius.PrescribedTemperature
- Modelica.Thermal.HeatTransfer.Celsius.TemperatureSensor
- Modelica.Thermal.HeatTransfer.Celsius.ToKelvin

### Thermal.HeatTransfer.Components (10 个)

- Modelica.Thermal.HeatTransfer.Components.BodyRadiation
- Modelica.Thermal.HeatTransfer.Components.Convection
- Modelica.Thermal.HeatTransfer.Components.ConvectiveResistor
- Modelica.Thermal.HeatTransfer.Components.GeneralHeatFlowToTemperatureAdaptor
- Modelica.Thermal.HeatTransfer.Components.GeneralTemperatureToHeatFlowAdaptor
- Modelica.Thermal.HeatTransfer.Components.HeatCapacitor
- Modelica.Thermal.HeatTransfer.Components.ThermalCollector
- Modelica.Thermal.HeatTransfer.Components.ThermalCollectorMatrix
- Modelica.Thermal.HeatTransfer.Components.ThermalConductor
- Modelica.Thermal.HeatTransfer.Components.ThermalResistor

### Thermal.HeatTransfer.Examples (4 个)

- Modelica.Thermal.HeatTransfer.Examples.ControlledTemperature
- Modelica.Thermal.HeatTransfer.Examples.GenerationOfFMUs
- Modelica.Thermal.HeatTransfer.Examples.Motor
- Modelica.Thermal.HeatTransfer.Examples.TwoMasses

### Thermal.HeatTransfer.Examples.Utilities (3 个)

- Modelica.Thermal.HeatTransfer.Examples.Utilities.Conduction
- Modelica.Thermal.HeatTransfer.Examples.Utilities.DirectCapacity
- Modelica.Thermal.HeatTransfer.Examples.Utilities.InverseCapacity

### Thermal.HeatTransfer.Fahrenheit (5 个)

- Modelica.Thermal.HeatTransfer.Fahrenheit.FixedTemperature
- Modelica.Thermal.HeatTransfer.Fahrenheit.FromKelvin
- Modelica.Thermal.HeatTransfer.Fahrenheit.PrescribedTemperature
- Modelica.Thermal.HeatTransfer.Fahrenheit.TemperatureSensor
- Modelica.Thermal.HeatTransfer.Fahrenheit.ToKelvin

### Thermal.HeatTransfer.Interfaces (4 个)

- Modelica.Thermal.HeatTransfer.Interfaces.Element1D
- Modelica.Thermal.HeatTransfer.Interfaces.PartialConditionalHeatPort
- Modelica.Thermal.HeatTransfer.Interfaces.PartialElementaryConditionalHeatPort
- Modelica.Thermal.HeatTransfer.Interfaces.PartialElementaryConditionalHeatPortWithoutT

### Thermal.HeatTransfer.Rankine (5 个)

- Modelica.Thermal.HeatTransfer.Rankine.FixedTemperature
- Modelica.Thermal.HeatTransfer.Rankine.FromKelvin
- Modelica.Thermal.HeatTransfer.Rankine.PrescribedTemperature
- Modelica.Thermal.HeatTransfer.Rankine.TemperatureSensor
- Modelica.Thermal.HeatTransfer.Rankine.ToKelvin

### Thermal.HeatTransfer.Sensors (4 个)

- Modelica.Thermal.HeatTransfer.Sensors.ConditionalFixedHeatFlowSensor
- Modelica.Thermal.HeatTransfer.Sensors.HeatFlowSensor
- Modelica.Thermal.HeatTransfer.Sensors.RelTemperatureSensor
- Modelica.Thermal.HeatTransfer.Sensors.TemperatureSensor

### Thermal.HeatTransfer.Sources (4 个)

- Modelica.Thermal.HeatTransfer.Sources.FixedHeatFlow
- Modelica.Thermal.HeatTransfer.Sources.FixedTemperature
- Modelica.Thermal.HeatTransfer.Sources.PrescribedHeatFlow
- Modelica.Thermal.HeatTransfer.Sources.PrescribedTemperature

### Utilities (2 个)

- Modelica.Utilities.Examples
- Modelica.Utilities.ReadRealMatrixFromFile

